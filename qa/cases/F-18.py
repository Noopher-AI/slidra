...
`元素「<id>」的 data-comot-media「<原樣的長字串>」副檔名「.xyz」不是支援
...
`<metadata><comot:effects .../></metadata>` 會留下（手動核對過，這是
...
rect 元素 id 與 `comot:effect ` 這個真實效果項目都不在了」，而不是逐位元
組比對。
"""
...
def main() -> int:  # noqa: PLR0915 - 單一線性流程，拆函式反而更難對照「造→量→還原→再量」四步
    ...
    if "comot:effect " in original:
        print(
            "FAIL slides/001.svg 開跑前就帶著既有的 comot:effect——這支腳本假設乾淨的"
    ...
    try:
        ...
        check("錯誤訊息包含合成的長 data-comot-media 值", long_media in (error_text or ""), (error_text or "")[:120])
        ...
        # 這個合成的壞資料留給下一支案例腳本。
        co_motion("effect", "remove", presentation_id, slide_path, "1")
        co_motion("element", "delete", presentation_id, slide_path, element_id)
    ...
    # `<comot:effects>` 容器（見檔頭docstring），這是既有行為，不是這支
    # 腳本的殘留。只確認插入的元素與這個合成效果都真的不在了。
    check("還原後找不到插入的元素 id", element_id not in restored, element_id in restored)
    check("還原後找不到 comot:effect 項目（空容器可以留著）", "comot:effect " not in restored, "comot:effect " in restored)
