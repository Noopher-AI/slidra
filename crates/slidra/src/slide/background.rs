...
pub const BACKGROUND_ROLE_ATTRIBUTE: &str = "data-slidra-role";
pub const BACKGROUND_ROLE: &str = "background";
pub const BACKGROUND_ELEMENT_ID: &str = "el-background";

fn svg_root(roots: &[ScannedNode]) -> SlidraResult<&ScannedNode> {
    roots
        .iter()
        .find(|n| n.tag == "svg")
...
pub fn clear_background(svg_content: &str) -> SlidraResult<String> {
    let roots = scan_document(svg_content)?;
    let root = svg_root(&roots)?;
...
pub fn lock_declared_background(svg_content: &str) -> SlidraResult<String> {
    let roots = scan_document(svg_content)?;
    let root = svg_root(&roots)?;
    let Some(existing) = find_background(root) else {
...
mod tests {
    ...
    const PAGE: &str = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1280 720\"><metadata><slidra:notes xmlns:slidra=\"https://slidra.app/ns/2026\">n</slidra:notes></metadata><g id=\"el-a\"><rect x=\"0\" y=\"0\" width=\"1\" height=\"1\"/></g></svg>";
    ...
    fn set_again_replaces_instead_of_stacking() {
        ...
        assert_eq!(twice.matches("data-slidra-role=\"background\"").count(), 1);
    ...
    fn lock_declared_background_adds_the_lock_once() {
        let page = "<svg viewBox=\"0 0 1280 720\"><g id=\"el-bg\" data-slidra-role=\"background\"><image x=\"0\" y=\"0\" width=\"1280\" height=\"720\" href=\"assets/a.svg\"/></g></svg>";
        let locked = lock_declared_background(page).unwrap();
        assert!(
            locked.contains("data-slidra-role=\"background\" data-slidra-lock=\"true\">"),
            "{locked}"
        );
