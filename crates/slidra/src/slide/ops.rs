...
/// source). Any `<slidra:effect target="...">` naming a remapped id is
...
pub fn mint_element_ids(
    ...
    // `<slidra:effect>` children only ever *reference* one via `target`,
...
/// `slidra slide add`. With `template_path`, the new slide is the
...
pub fn add_slide(id: &str, input: AddSlideInput) -> SlidraResult<AddSlideResult> {
...
/// `slidra slide delete`. Allowed to bring `slides` down to zero, and
...
/// `slidra slide duplicate`: inserted directly after the source; every
/// element id (and nothing else) is re-minted; `<slidra:notes>` is copied
/// verbatim along with the rest of the document.
pub fn duplicate_slide(id: &str, slide_path: &str) -> SlidraResult<DuplicateSlideResult> {
    let project = read_project_json(&workspace::resolve_work_dir(id)?)?;
...
/// `slidra slide move`: rewrites only `project.json`'s `slides` order —
/// no SVG file is opened or written.
pub fn move_slide(id: &str, slide_path: &str, new_index: f64) -> SlidraResult<()> {
    let project = read_project_json(&workspace::resolve_work_dir(id)?)?;
...
/// `slidra template add`: registers the new path in `project.json`'s
/// `templates` (created if this is the presentation's first template) —
/// never in `slides`.
pub fn add_template(id: &str, input: AddTemplateInput) -> SlidraResult<AddTemplateResult> {
...
/// `slidra template list`: the presentation's templates in
/// `project.json` order, normalized.
pub fn list_templates(id: &str) -> SlidraResult<Vec<TemplateEntry>> {
    let project = read_project_json(&workspace::resolve_work_dir(id)?)?;
    Ok(read_template_entries(&project))
}

/// `slidra template rename`: renames a template's `name` field only —
...
/// `slidra slide notes set` — slides only, never templates.
...
/// `slidra slide transition set`. Reads `slide_path`'s current
...
mod tests {
    ...
    fn mint_element_ids_remaps_ids_and_effect_targets() {
        let svg = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\"><rect id=\"a\" width=\"1\"/><metadata><slidra:effects xmlns:slidra=\"https://slidra.app/ns/2026\"><slidra:effect target=\"a\" family=\"enter\"/></slidra:effects></metadata></svg>";
        let mut counter = 0;
        let mut gen_fn = move || {
            counter += 1;
            format!("new-{counter}")
    ...
    /// `crates/slidra/tests/cli_golden.rs`'s
