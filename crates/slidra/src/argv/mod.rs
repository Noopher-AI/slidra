...
//! functions build on `SlidraResult` and propagate with `?` — NOOP-281/F5
...
//! `SlidraError` contract. `collect_repeated_flag`/`parse_js_number` are
...
/// `SlidraError` instead of `String` — see this module's own doc comment
