...
use super::runs::{TextRun, utf16_len, utf16_slice};
...
/// `data-slidra-break="1"` on a line that ends on a hard break, and — when
...
pub fn render_text_box_content(lines: &[WrappedLine], runs: &[TextRun]) -> String {
    ...
    for line in lines {
        ...
        let break_attr = if line.hard_break {
            " data-slidra-break=\"1\""
        } else {
            ""
        };
...
fn render_line_runs(text: &str, line_start: usize, runs: &[TextRun]) -> String {
    ...
    let mut relevant: Vec<&TextRun> = runs
        .iter()
        .filter(|run| run.start < line_end && run.end > line_start)
        .collect();
    ...
    for run in relevant {
        ...
        if seg_start > pos {
            result.push_str(&escape_xml_text(&utf16_slice(
                text,
                pos - line_start,
...
mod tests {
    ...
    fn hard_break_line_gets_the_data_attribute_non_hard_break_does_not() {
        let lines = vec![
            line("line one", 0.0, 27.84, true),
            line("line two", 0.0, 62.592, false),
        ];
        let out = render_text_box_content(&lines, &[]);
        assert_eq!(
            out,
            "<tspan x=\"0\" y=\"27.84\" data-slidra-break=\"1\">line one</tspan><tspan x=\"0\" y=\"62.592\">line two</tspan>"
        );
    }
    ...
    fn no_whitespace_between_adjacent_tspans() {
        ...
        assert_eq!(
            out,
            "<tspan x=\"0\" y=\"10\">a</tspan><tspan x=\"0\" y=\"20\">b</tspan>"
        );
    }
    ...
    fn run_covering_the_whole_line_wraps_it_in_one_nested_tspan() {
        ...
        assert_eq!(
            out,
            "<tspan x=\"0\" y=\"10\"><tspan font-weight=\"bold\">hello</tspan></tspan>"
        );
    }
    ...
    fn run_covering_part_of_a_line_splits_into_plain_and_nested_segments() {
        ...
        assert_eq!(
            out,
            "<tspan x=\"0\" y=\"10\">AA<tspan font-style=\"italic\">BB</tspan>CC</tspan>"
        );
    }
    ...
    fn hard_break_advances_the_cursor_by_one_extra_utf16_unit_for_the_next_line() {
        ...
        assert_eq!(
            out,
            "<tspan x=\"0\" y=\"10\" data-slidra-break=\"1\">AA</tspan><tspan x=\"0\" y=\"20\"><tspan font-weight=\"bold\">BB</tspan></tspan>"
        );
    }
