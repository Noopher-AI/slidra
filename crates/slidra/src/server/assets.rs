...
//! (`FileEntryPolicy.remoteUrl`/`uploadBytes`) is injected when the deck
//! server starts and enforced at this reachable browser boundary. Everything
//! downstream of that — header validation, the
...
pub(crate) struct AssetPolicy {
    pub(crate) upload_bytes: bool,
    pub(crate) remote_url: bool,
}

pub(crate) fn handle(request: &RawRequest, policy: &AssetPolicy, stream: &mut TcpStream) {
    ...
    if server::editing_lock::is_frozen(&credential.workbench_id) {
    ...
    if has_url && !policy.remote_url {
        server::write_json_error(stream, 403, "Remote URL uploads are disabled");
        return;
    }
    if !has_url && !policy.upload_bytes {
        server::write_json_error(stream, 403, "Byte uploads are disabled");
        return;
    }
