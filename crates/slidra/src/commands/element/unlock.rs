...
//! `slidra element unlock` argv layer.
...
fn try_run(args: &[String]) -> SlidraResult<CommandResult> {
    let id = require_id_positional(args, 0, "element unlock", "presentation-id")?.to_string();
    ...
    Ok(CommandResult::success(
        format!("unlocked {} elements in {slide_path}", element_ids.len()),
