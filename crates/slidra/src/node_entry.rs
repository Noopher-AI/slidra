...
//! names (and only those two), this binary sets `SLIDRA_BIN` to its own
...
const NODE_ENTRY_RELATIVE: &str = "packages/server/bin/slidra-node.js";

/// Execs `node <repo>/packages/server/bin/slidra-node.js <argv>`,
...
pub fn exec_node(argv: &[OsString]) -> i32 {
    ...
    let current_exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("slidra"));

    let mut command = Command::new("node");
    command
        .arg(&entry)
        .args(argv)
        .env("SLIDRA_BIN", current_exe)
        .stdin(Stdio::inherit())
...
/// contains BOTH `Cargo.toml` and `packages/server/bin/slidra-node.js`.
