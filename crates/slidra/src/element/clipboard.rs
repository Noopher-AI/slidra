...
) -> SlidraResult<Option<&'a ScannedNode>> {
...
fn rewrite_fragment_transform(
    markup: &str,
    mutate: impl FnOnce(TransformParts) -> SlidraResult<TransformParts>,
) -> SlidraResult<String> {
...
/// Extracts a copyable snapshot of every target (`slidra element copy`).
...
/// `<slidra:effect>` whose `target` names the copied element or any of its
/// descendants is captured verbatim, in document order.
pub fn extract_elements_for_copy(
    svg_content: &str,
    slide_path: &str,
    element_ids: &[String],
) -> SlidraResult<ClipboardPayload> {
    ...
    if let Some(metadata) = svg_root.children.iter().find(|c| c.tag == "metadata") {
        if let Some(effects_list) = metadata.children.iter().find(|c| c.tag == "slidra:effects") {
            for effect in &effects_list.children {
                if effect.tag != "slidra:effect" {
                    continue;
                }
...
) -> SlidraResult<(String, HashMap<String, String>)> {
    let roots = scan_document(markup)?;
    let root = roots
        .first()
        .ok_or_else(|| SlidraError::invalid("clipboard fragment is empty"))?;
...
/// list — creating `<metadata><slidra:effects>` if the slide has none yet.
fn append_effects(svg_content: &str, effect_markups: &[String]) -> SlidraResult<String> {
    let roots = scan_document(svg_content)?;
    let svg_root = require_svg_root(&roots)?;
    ...
    if let Some(metadata) = svg_root.children.iter().find(|c| c.tag == "metadata") {
        ...
        let block =
            format!("<slidra:effects xmlns:slidra=\"{EFFECTS_NS}\">{joined}</slidra:effects>");
        return Ok(splice_insert(svg_content, metadata.content_end, &block));
    }
    let block = format!(
        "<metadata><slidra:effects xmlns:slidra=\"{EFFECTS_NS}\">{joined}</slidra:effects></metadata>"
    );
...
pub struct ClipboardPayload {
    ...
    /// `<slidra:effect>` markup, verbatim, in original document order.
...
const CLIPBOARD_MARKER_ATTR: &str = "data-slidra-clipboard";
const CLIPBOARD_MARKER_VALUE: &str = "elements";
const CLIPBOARD_SOURCE_ATTR: &str = "data-slidra-source";
...
/// Free-form display text (`data-slidra-name`) — excludes only control
...
/// `data-slidra-list`: one whitespace(single-space)-separated token per
...
fn common_attributes() -> HashMap<&'static str, Grammar> {
    ...
    m.insert("data-slidra-name", grammar(is_free_text));
...
fn element_tag_attributes() -> &'static HashMap<&'static str, HashMap<&'static str, Grammar>> {
    ...
    MAP.get_or_init(|| {
        ...
        g.insert("data-slidra-name", grammar(is_free_text));
        g.insert("data-slidra-media", grammar(is_relative_ref));
        g.insert("data-slidra-lock", enum_of(&["true"]));
        g.insert("data-slidra-type", enum_of(&["table"]));
        g.insert("data-slidra-cell", grammar(is_cell_ref));
        g.insert("data-slidra-cols", number_list(None));
        g.insert("data-slidra-rows", number_list(None));
        g.insert("data-slidra-header", enum_of(&["1"]));
        g.insert("data-slidra-theme", enum_of(&["dark", "light", "zebra"]));
        g.insert("data-slidra-span", grammar(is_cell_ref));
        g.insert("data-slidra-repeat", enum_of(&["row"]));
        g.insert("data-slidra-generated", enum_of(&["1"]));
        g.insert("data-slidra-align", enum_of(&["left", "center", "right"]));
        g.insert("display", enum_of(&["none"]));
        g.insert("data-slidra-text-width", grammar(is_number));
        g.insert("data-slidra-text-height", grammar(is_number));
        g.insert(
        ...
        circle.insert("data-slidra-media", grammar(is_relative_ref));
        ...
        image.insert("data-slidra-media", grammar(is_relative_ref));
        ...
        text.insert("data-slidra-list", grammar(is_list_spec));
        text.insert("data-slidra-list-marker", enum_of(&["true"]));
        m.insert("text", text);
        ...
        tspan.insert("data-slidra-break", enum_of(&["1"]));
...
/// The fixed effect value sets a `<slidra:effect>` grammar validates against
...
fn check_node_attributes(node: &ScannedNode, label: &str, is_effect: bool) -> SlidraResult<()> {
    let tag_grammar: Option<&HashMap<&str, Grammar>> = if is_effect {
        if node.tag == "slidra:effect" {
...
pub fn sanitize_clipboard_markup(markup: &str, kind: ClipboardMarkupKind) -> SlidraResult<()> {
    ...
    let wrapped = match kind {
        ...
        ClipboardMarkupKind::Effect => format!(
            "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"{SANITIZE_PLACEHOLDER_VIEWBOX}\"><metadata><slidra:effects xmlns:slidra=\"{EFFECTS_NS}\">{markup}</slidra:effects></metadata></svg>"
        ),
    };
    ...
    let expected_root_tag = match kind {
        ...
        ClipboardMarkupKind::Effect => "slidra:effect",
    };
...
pub fn serialize_clipboard_svg(payload: &ClipboardPayload) -> SlidraResult<String> {
    ...
    let effects_block = if !payload.effects.is_empty() {
        format!(
            "<metadata><slidra:effects xmlns:slidra=\"{EFFECTS_NS}\">{}</slidra:effects></metadata>",
    ...
    Ok(format!(
        "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:slidra=\"{EFFECTS_NS}\" viewBox=\"{view_box}\" {CLIPBOARD_MARKER_ATTR}=\"{CLIPBOARD_MARKER_VALUE}\" {CLIPBOARD_SOURCE_ATTR}=\"{}\">{effects_block}{}</svg>",
...
/// errors — when `markup` is not a slidra element clipboard payload at
...
mod tests {
    ...
    fn sanitize_accepts_a_well_formed_effect_fragment() {
        let markup = r#"<slidra:effect target="el-a" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/>"#;
    ...
    fn sanitize_effect_rejects_an_unsupported_family_or_effect_name() {
        let bad_family = r#"<slidra:effect target="el-a" family="bogus" effect="fade" start="on-click" duration="0.4" delay="0"/>"#;
        assert!(sanitize_clipboard_markup(bad_family, ClipboardMarkupKind::Effect).is_err());
        let bad_effect = r#"<slidra:effect target="el-a" family="enter" effect="bogus" start="on-click" duration="0.4" delay="0"/>"#;
    ...
    fn extract_captures_only_effects_targeting_a_copied_element_or_its_descendant() {
        let svg = slide(&format!(
            r#"<metadata><slidra:effects xmlns:slidra="{EFFECTS_NS}"><slidra:effect target="el-a" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/><slidra:effect target="el-b" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/></slidra:effects></metadata><g id="el-a"><rect x="0" y="0" width="1" height="1"/></g><g id="el-b"><rect x="0" y="0" width="1" height="1"/></g>"#
        ));
        let payload =
            extract_elements_for_copy(&svg, "slides/001.svg", &["el-a".to_string()]).unwrap();
    ...
    fn paste_rewrites_effect_targets_through_the_regenerated_id_map() {
        ...
        let payload = ClipboardPayload {
            ...
            effects: vec![
                r#"<slidra:effect target="el-old" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/>"#
                    .to_string(),
            ],
            view_box: None,
        };
