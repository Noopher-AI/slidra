...
pub fn set_attr_splice(node: &ScannedNode, attr: &str, value: &str) -> Splice {
    ...
    // and `slidra:*` tag names), so `.len()` (bytes) and UTF-16 length
...
///
/// `mutate` is fallible (`SlidraResult`, not a bare `TransformParts`)
...
mod tests {
    ...
    fn set_attr_splice_inserts_after_tag_name_when_absent() {
        ...
        let roots = scan_document(svg).unwrap();
        let node = &roots[0];
        let splice = set_attr_splice(node, "data-slidra-lock", "true");
        let updated = apply_splices(svg, &[splice]);
        assert_eq!(updated, r#"<g data-slidra-lock="true"><rect/></g>"#);
    }

    #[test]
    fn set_attr_splice_replaces_existing_value() {
        let svg = r#"<g data-slidra-lock="false"><rect/></g>"#;
        let roots = scan_document(svg).unwrap();
        let node = &roots[0];
        let splice = set_attr_splice(node, "data-slidra-lock", "true");
        let updated = apply_splices(svg, &[splice]);
    ...
    fn attribute_removal_splice_also_removes_the_leading_space() {
        let svg = r#"<g id="x" data-slidra-lock="true"><rect/></g>"#;
        let roots = scan_document(svg).unwrap();
        let node = &roots[0];
        let attr = attribute_of(node, "data-slidra-lock").unwrap();
