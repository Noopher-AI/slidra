...
//! `svg_content: &str -> SlidraResult<String>` functions: every target is
...
fn require_finite_number(value: Option<f64>, flag: &str) -> SlidraResult<f64> {
    match value {
...
fn require_positive_number(value: Option<f64>, flag: &str) -> SlidraResult<f64> {
...
/// supports, deliberately without expanding the set of supported
/// platforms — `element insert` never resolves a YouTube URL itself, only
/// validates and stores the provider name the caller already resolved it
/// to.
fn require_embed_provider(value: &str) -> SlidraResult<&'static str> {
    match value {
        "youtube" => Ok("youtube"),
        _ => Err(SlidraError::invalid(format!(
...
/// Builds and appends one new element (`slidra element insert`).
...
pub fn insert_element(
    ...
    if let Some(embed) = input.embed.as_deref() {
        ...
        container_attrs.push_str(&format!(" data-slidra-embed=\"{provider}\""));
    }
    ...
    let native = match kind {
        ...
        InsertElementKind::Video | InsertElementKind::Audio => {
            ...
            let width = require_positive_number(input.width, "width")?;
            let height = require_positive_number(input.height, "height")?;
            container_attrs.push_str(&format!(" data-slidra-type=\"{}\"", kind.as_str()));
            container_attrs.push_str(&format!(
                " transform=\"translate({} {})\"",
                format_svg_number(x),
                format_svg_number(y)
...
fn find_dangling_effect_ranges(
    ...
    let Some(effects_list) = metadata
        .children
        .iter()
        .find(|child| child.tag == "slidra:effects")
    else {
        return Vec::new();
    };
    effects_list
...
/// [E2.T8]'s `<slidra:comments>` list, same shape as
...
fn find_dangling_comment_ranges(
    ...
    let Some(comments_list) = metadata
        .children
        .iter()
        .find(|child| child.tag == "slidra:comments")
    else {
        return Vec::new();
    };
    comments_list
        .children
        .iter()
        .filter(|child| child.tag == "slidra:comment")
        .filter_map(|comment| {
...
/// Deletes every element named in `element_ids` (`slidra element
...
/// its own container's `transform` (`slidra element move`).
...
/// independently (`slidra element rotate`). A target whose existing
...
fn move_to_edge(svg: &str, id: &str, to_front: bool, force: bool) -> SlidraResult<String> {
    let roots = scan_document(svg)?;
    let svg_root = require_svg_root(&roots)?;
...
fn move_one_step(svg: &str, id: &str, up: bool, force: bool) -> SlidraResult<String> {
    let roots = scan_document(svg)?;
    let svg_root = require_svg_root(&roots)?;
    let (node, parent) = require_container(svg_root, id)?;
    assert_not_locked(node, id, force)?;
...
/// Reorders every target within its own parent container (`slidra
...
/// Removes `data-slidra-lock` from every target's container (`slidra
/// element unlock`) — unlocking clears the attribute entirely, it never
/// writes `data-slidra-lock="false"`. Idempotent: unlocking an
...
/// A chart container's children (`<slidra:chart>`, the data; `<svg>`, the
...
/// "unsupported primitive `<slidra:chart>`".
...
fn assert_subtree_not_locked(svg: &str, id: &str, force: bool) -> SlidraResult<()> {
    let roots = scan_document(svg)?;
    let svg_root = require_svg_root(&roots)?;
    let (node, _parent) = require_container(svg_root, id)?;
    assert_not_locked(node, id, force)?;
...
fn assert_positive_after_rounding(value: f64, message: String) -> SlidraResult<f64> {
    let rounded: f64 = format_svg_number(value)
        .parse()
        .expect("format_svg_number always produces a string that reparses as f64");
    if rounded <= 0.0 {
        return Err(SlidraError::invalid(message));
    }
    Ok(rounded)
}
...
/// `grep -rn CHART_CONTAINER_TYPE crates/slidra/src` finding only the
...
fn scale_special_container(
    ...
    sy: f64,
    force: bool,
) -> SlidraResult<Option<String>> {
    let container_type = attribute_value(node, "data-slidra-type");
...
/// (`data-slidra-text-width` present) re-wraps its content at the scaled
...
/// Scales every target by the same `factor` (`slidra element scale`).
...
/// Resizes every target to the same `(width, height)` (`slidra element
/// resize`).
...
/// Sets `attr` to `value` on every target's primitive (`slidra element
...
mod tests {
    ...
    fn delete_removes_element_and_dangling_effect_and_comment() {
        let svg = slide(
            r#"<metadata><slidra:effects xmlns:slidra="https://slidra.app/ns/2026"><slidra:effect target="el-a" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/></slidra:effects><slidra:comments xmlns:slidra="https://slidra.app/ns/2026"><slidra:comment id="c-1" target="el-a" author="agent" created="2024-01-01T00:00:00.000Z">hi</slidra:comment></slidra:comments></metadata><g id="el-a"><rect x="0" y="0" width="1" height="1"/></g>"#,
        );
        let updated = delete_elements(&svg, "slides/001.svg", &["el-a".to_string()]).unwrap();
        assert!(!updated.contains("el-a"));
        assert!(!updated.contains("slidra:effect "));
    ...
    fn lock_is_idempotent_and_unlock_removes_the_attribute_entirely() {
        ...
        assert!(locked.contains(r#"data-slidra-lock="true""#));
    ...
    fn scale_rejects_a_locked_descendant_even_though_only_the_outer_group_was_named() {
        let svg = slide(
            r#"<g id="grp"><g id="child" data-slidra-lock="true"><rect x="0" y="0" width="1" height="1"/></g></g>"#,
        );
    ...
    fn scale_chart_container_is_an_explicit_failed_naming_noop_281() {
        let svg = slide(r#"<g id="c" data-slidra-type="chart"><slidra:chart/><svg/></g>"#);
        let err = scale_elements(
            &svg,
            "slides/001.svg",
            &["c".to_string()],
    ...
    fn resize_text_box_uniformly_delegates_to_scale() {
        let svg = slide(&format!(
            r##"<g id="tb" {TEXT_WIDTH_ATTRIBUTE}="100" data-slidra-text-height="25"><text font-family="Noto Sans TC" font-size="16" xml:space="preserve"><tspan x="0" y="0">hi</tspan></text></g>"##
        ));
        let updated = resize_elements(
            &svg,
            "slides/001.svg",
    ...
    #[test]
    fn style_set_rejects_data_slidra_prefixed_attributes() {
        let svg = slide(r#"<g id="a"><rect x="0" y="0" width="1" height="1"/></g>"#);
        let err = set_element_style(
            &svg,
            "slides/001.svg",
