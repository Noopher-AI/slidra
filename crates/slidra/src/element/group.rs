...
fn is_group_container(node: &ScannedNode) -> bool {
    if attribute_value(node, "data-slidra-type").as_deref()
...
fn collect_data_slidra_names(node: &ScannedNode, names: &mut Vec<String>) {
    if let Some(name) = attribute_value(node, "data-slidra-name") {
        names.push(name);
    }
    for child in &node.children {
        collect_data_slidra_names(child, names);
    }
}
...
fn next_group_name(svg_root: &ScannedNode) -> String {
    ...
    collect_data_slidra_names(svg_root, &mut names);
    let next = names
        .iter()
...
pub fn group_elements(
    ...
    element_ids: &[String],
    new_group_id: &str,
) -> SlidraResult<GroupElementsResult> {
    ...
    let mut combined_markup = format!(
        "<g id=\"{new_group_id}\" data-slidra-name=\"{}\">",
...
/// group's own `data-slidra-name`/`data-slidra-media` disappear with it.
///
...
fn ungroup_one(svg_content: &str, slide_path: &str, id: &str) -> SlidraResult<UngroupOneResult> {
    let roots = scan_document(svg_content)?;
    let svg_root = require_svg_root(&roots)?;
...
/// Ungroups every target (`slidra element ungroup`). Multiple targets
...
pub fn ungroup_elements(
    ...
    slide_path: &str,
    element_ids: &[String],
) -> SlidraResult<UngroupElementsResult> {
    assert_slide_compliant(svg_content, slide_path)?;
    validate_id_list(element_ids)?;
...
/// Sets `data-slidra-name` on every target container (`slidra element
...
pub fn set_element_name(
    ...
    for id in element_ids {
        ...
        let svg_root = require_svg_root(&roots)?;
        let (node, _parent) = require_container(svg_root, id)?;
        let splice = name_attr_splice(&current, node, "data-slidra-name", name);
...
mod tests {
    ...
    fn group_wraps_targets_at_topmost_position_in_document_order() {
        ...
        .unwrap();
        let expected = slide(
            r#"<g id="mid"><rect x="0" y="0" width="1" height="1"/></g><g id="el-group1" data-slidra-name="Group 1"><g id="a"><rect x="0" y="0" width="1" height="1"/></g><g id="b"><rect x="0" y="0" width="1" height="1"/></g></g>"#,
        );
    ...
    fn group_name_numbering_never_fills_a_gap() {
        let svg = slide(
            r#"<g id="a" data-slidra-name="Group 1"><rect x="0" y="0" width="1" height="1"/></g><g id="b" data-slidra-name="Group 5"><rect x="0" y="0" width="1" height="1"/></g><g id="c"><rect x="0" y="0" width="1" height="1"/></g>"#,
        );
        let result = group_elements(
            &svg,
            "slides/001.svg",
        ...
        .unwrap();
        assert!(result.svg.contains(r#"data-slidra-name="Group 6""#));
    }

    #[test]
    fn group_removes_effects_directly_targeting_members() {
        let svg = slide(&format!(
            r#"<metadata><slidra:effects xmlns:slidra="{}"><slidra:effect target="a" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/></slidra:effects></metadata><g id="a"><rect x="0" y="0" width="1" height="1"/></g><g id="b"><rect x="0" y="0" width="1" height="1"/></g>"#,
        ...
        assert!(!result.svg.contains("slidra:effect "));
    }
    ...
    fn ungroup_folds_group_matrix_into_each_child_and_reports_child_ids() {
        let svg = slide(
            r#"<g id="grp" data-slidra-name="Group 1" transform="translate(100 100)"><g id="a" transform="translate(5 5)"><rect x="0" y="0" width="1" height="1"/></g><g id="b"><rect x="0" y="0" width="1" height="1"/></g></g>"#,
        );
        ...
        assert!(!result.svg.contains("data-slidra-name"));
    ...
    fn ungroup_removes_effect_targeting_the_group_itself() {
        let svg = slide(&format!(
            r#"<metadata><slidra:effects xmlns:slidra="{}"><slidra:effect target="grp" family="enter" effect="fade" start="on-click" duration="0.4" delay="0"/></slidra:effects></metadata><g id="grp"><g id="a"><rect x="0" y="0" width="1" height="1"/></g><g id="b"><rect x="0" y="0" width="1" height="1"/></g></g>"#,
            crate::effects::edit::EFFECTS_NS
        ));
        let result = ungroup_elements(&svg, "slides/001.svg", &["grp".to_string()]).unwrap();
        assert_eq!(result.removed_effects, 1);
