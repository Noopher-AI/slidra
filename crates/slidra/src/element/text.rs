...
/// `data-slidra-text-align` (NOOP-65 決定 D). Duplicated as a literal rather
...
/// `data-slidra-list` (NOOP-65 決定 E): one whitespace-separated token per
/// paragraph, on the content `<text>` (not the container).
const LIST_ATTRIBUTE: &str = "data-slidra-list";

/// `data-slidra-list-marker="true"`: marks the second `<text>` a text box's
...
const LIST_MARKER_ATTRIBUTE: &str = "data-slidra-list-marker";
...
/// Reads `data-slidra-list` off the content `<text>`, padded/truncated to
...
/// adds `data-slidra-text-height` to an EXISTING container (one that has
...
fn resolve_font<'a>(
    ...
    font_family: &str,
    element_id: &str,
) -> SlidraResult<&'a dyn FontMetrics> {
    fonts
        .get(font_family)
...
fn assert_valid_xml_text(new_text: &str) -> SlidraResult<()> {
...
fn render_plain_text_content(
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        // `data-slidra-break="1"` — the same marker `read_text_box_runs`
...
fn replace_container_text(
    ...
    if let Some(text_width_raw) = text_width_raw {
        ...
        // an author gets by pressing Enter at the end of a list item.
        let (old_text, _) = read_text_box_runs(text_node, svg_content);
        let old_tokens = read_list_tokens(text_node, old_text.split('\n').count(), element_id)?;
        let paragraph_count = new_text.split('\n').count();
        let list_tokens: Vec<ListKind> = (0..paragraph_count)
            .map(|i| {
                old_tokens
                    .get(i)
                    .or_else(|| old_tokens.last())
                    .copied()
...
/// (`slidra text set`). Does NOT call `assert_slide_compliant` (see
/// module doc comment).
...
/// `font_size` and splices the container's `data-slidra-text-width`/
/// `data-slidra-text-height` attributes and the `<text>`'s content into
...
/// (`slidra textbox width`). The text itself is unchanged; only
/// `data-slidra-text-width` and the baked-in `<tspan>`s move.
...
/// `slidra textbox align`: sets a text box's `data-slidra-text-align` and
...
/// `slidra text style set`'s mutation primitive: sets or clears
...
/// `slidra text list set`'s mutation primitive: sets one paragraph's
...
/// Builds the markup of one text box — the `<g>` container carrying
/// `data-slidra-text-width`/`-height` and the translate, the content
...
pub(crate) fn build_text_box_markup(
    ...
    let paragraph_count = input.text.split('\n').count();
    let tokens: Vec<ListKind> = (0..paragraph_count)
        .map(|i| list_tokens.get(i).copied().unwrap_or(ListKind::None))
        .collect();
    let has_list = tokens.iter().any(|token| *token != ListKind::None);
    let indents = list_indents(&tokens, normalized_font_size);
    let wrapped = wrap_text(
        input.text,
        &WrapOptions {
            ...
            indents: if has_list { Some(&indents) } else { None },
        },
    )?;
    let content = render_text_box_content(&wrapped.lines, &[]);

    let name_attr = name
        .map(|n| format!(" data-slidra-name=\"{}\"", escape_xml_attr(n)))
        .unwrap_or_default();
    let list_attr = if has_list {
        format!(
            " {LIST_ATTRIBUTE}=\"{}\"",
            tokens
                .iter()
    ...
    let markup = format!(
        "<g id=\"{element_id}\"{name_attr} {TEXT_WIDTH_ATTRIBUTE}=\"{}\" {TEXT_HEIGHT_ATTRIBUTE}=\"{}\"{align_attr} transform=\"translate({} {})\">\
<text{list_attr} font-family=\"{}\" font-size=\"{}\"{weight_attr}{fill_attr} xml:space=\"preserve\">{content}</text>{marker}</g>",
    ...
    Ok((markup, wrapped.lines.len()))
}

/// Creates a new text box on a slide: a `<g>` container carrying
/// `data-slidra-text-width`/`-height` and the translate, wrapping a
/// `<text>` with one `<tspan>` per rendered line. Appended as the last
/// child of the root `<svg>`. Returns `(updated svg, rendered line count)`.
pub fn add_text_box(
    svg_content: &str,
    element_id: &str,
    input: &AddTextBoxInput,
    ...
    let (markup, lines) = build_text_box_markup(element_id, None, input, &[], fonts)?;
    let updated = append_markup(svg_content, &markup)?;
    Ok((updated, lines))
}

fn append_markup(svg_content: &str, markup: &str) -> SlidraResult<String> {
    let roots = scan_document(svg_content)?;
    let svg_root = roots
        .iter()
...
mod tests {
    ...
    /// `<tspan>` per paragraph, `data-slidra-break="1"` on every tspan but
    ...
    fn textbox_svg(width: &str, paragraphs: &[&str]) -> String {
        ...
        slide(&format!(
            r#"<g id="tb" data-slidra-text-width="{width}"><text font-family="Noto Sans TC" font-size="16" xml:space="preserve">{tspans}</text></g>"#
        ))
    }
    ...
    fn text_set_on_a_text_box_rewraps_and_keeps_its_list_state() {
        ...
        let svg = slide(
            r#"<g id="tb" data-slidra-text-width="200"><text font-family="Noto Sans TC" font-size="16" data-slidra-list="bullet" xml:space="preserve"><tspan x="0" y="0">old</tspan></text><text data-slidra-list-marker="true" font-family="Noto Sans TC" font-size="16" xml:space="preserve"><tspan x="0" y="0">•</tspan></text></g>"#,
        );
        let updated = replace_element_text(&svg, "tb", "new text", &font_book(), false).unwrap();
        assert!(updated.contains(">new text<"), "{updated}");
        assert!(
            updated.contains(r#"data-slidra-list="bullet""#),
            "{updated}"
        );
        assert_eq!(
            updated.matches("data-slidra-list-marker").count(),
            1,
            "{updated}"
        );
        assert!(updated.contains("data-slidra-text-height="), "{updated}");
    }
    ...
    /// case `text set / textbox-full-replace-keeps-list-state`):
    ...
    /// (the one that used to separate `data-slidra-list="bullet"` from its
    ...
    fn text_set_full_replace_keeps_the_list_and_extends_it_to_new_paragraphs() {
        ...
        let svg = slide(
            r#"<g id="tb" data-slidra-text-width="200"><text font-family="Noto Sans TC" font-size="16" data-slidra-list="bullet" xml:space="preserve"><tspan x="0" y="0">old</tspan></text></g>"#,
        );
        let updated =
            replace_element_text(&svg, "tb", "one\ntwo\nthree", &font_book(), false).unwrap();
        assert!(
            updated.contains(r#"data-slidra-list="bullet bullet bullet""#),
            "{updated}"
        );
        assert!(updated.contains("data-slidra-list-marker"), "{updated}");
    }

    #[test]
    fn text_set_full_replace_on_a_box_without_a_list_writes_no_list_attribute() {
        let svg = textbox_svg("200", &["old"]);
        let updated = replace_element_text(&svg, "tb", "new text", &font_book(), false).unwrap();
        assert!(!updated.contains("data-slidra-list"), "{updated}");
    }
    ...
    fn text_set_on_text_box_with_malformed_width_errors() {
        ...
        assert!(err.message().contains("data-slidra-text-width"));
    }
    ...
    fn textbox_width_rewraps_at_the_new_width() {
        ...
        assert!(
            updated.contains(r#"data-slidra-text-width="50""#),
            "{updated}"
        );
        assert!(lines >= 1);
    }
    ...
    fn textbox_align_writes_the_attribute_and_rewraps() {
        ...
        assert!(
            updated.contains(r#"data-slidra-text-align="center""#),
            "{updated}"
        );
    }
    ...
    fn textbox_align_left_after_center_removes_the_attribute_on_the_next_left_call() {
        ...
        // still leaves a literal `data-slidra-text-align="left"` attribute,
        // it does not remove it the way `textbox add`'s own left-is-default
        // omission does at creation time.
        assert!(
            left_again.contains(r#"data-slidra-text-align="left""#),
            "{left_again}"
        );
    }
    ...
    fn text_list_set_bullet_writes_the_list_attribute_and_a_marker() {
        ...
        assert!(
            updated.contains(r#"data-slidra-list="bullet none""#),
            "{updated}"
        );
        assert!(updated.contains("data-slidra-list-marker"), "{updated}");
    }
    ...
    fn text_list_set_all_none_after_removing_the_last_list_paragraph_clears_the_attribute() {
        ...
        assert!(!cleared.contains("data-slidra-list="), "{cleared}");
        assert!(!cleared.contains("data-slidra-list-marker"), "{cleared}");
        // Regression: clearing must remove the ONE space that used to separate
        // `data-slidra-list="bullet"` from its neighbours, not leave it
    ...
    fn textbox_add_creates_a_text_box_with_declared_width_and_wraps_content() {
        ...
        assert_eq!(lines, 1);
        assert!(
            updated.contains(r#"<g id="el-new" data-slidra-text-width="300""#),
            "{updated}"
        );
        assert!(
        ...
        assert!(!updated.contains("data-slidra-text-align"), "{updated}");
    }
