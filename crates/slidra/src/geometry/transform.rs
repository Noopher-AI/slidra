...
/// Ports the `ARITY` table; returns the SlidraError `parseArguments`
...
fn arity_for(name: &str) -> SlidraResult<&'static [usize]> {
    match name {
        "matrix" => Ok(&[6]),
        "translate" => Ok(&[1, 2]),
        "scale" => Ok(&[1, 2]),
...
pub fn parse_transform(value: Option<&str>) -> SlidraResult<Matrix> {
...
fn parse_arguments(name: &str, args_text: &str) -> SlidraResult<Vec<f64>> {
...
/// `arity_for` accepts is handled above) but returns the same "unsupported
/// transform function" error rather than panicking if the two tables ever
/// drift out of sync, so a future drift produces a clean error instead of
/// a crash.
fn function_to_matrix(name: &str, args: &[f64]) -> SlidraResult<Matrix> {
    match name {
        "matrix" => Ok(Matrix {
            a: args[0],
            b: args[1],
...
pub fn decompose_matrix(m: &Matrix) -> SlidraResult<TransformParts> {
