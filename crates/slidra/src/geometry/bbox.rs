...
fn number_attr(primitive: &SlidePrimitive, name: &str, fallback: Option<f64>) -> SlidraResult<f64> {
...
fn text_bounds(primitive: &SlidePrimitive, context: &TextBoundsContext<'_>) -> SlidraResult<Bbox> {
...
fn tokenize_path_data(d: &str) -> SlidraResult<Vec<PathTok>> {
...
fn next_number(tokens: &[PathTok], index: &mut usize, command: &str) -> SlidraResult<f64> {
...
pub fn path_bounds(d: &str) -> SlidraResult<Bbox> {
...
fn bounds_within(
    ...
    let measurable: Vec<&SlidePrimitive> = element
        ...
        .iter()
        .filter(|primitive| {
            primitive.attr(LIST_MARKER_ATTRIBUTE) != Some("true") && primitive.tag != "slidra:chart"
        })
        .collect();
