...
//!
//! Serializes a `TableModel` into the full `<g data-slidra-type="table">…</g>`
...
pub fn render_table_markup(
    ...
    let source_markup = match &model.source {
        Some(source) => format!(
            "<{TABLE_SOURCE_TAG} xmlns:slidra=\"{TABLE_NS}\" src=\"{}\"/>",
    ...
    Ok(format!(
        "<g id=\"{}\" data-slidra-type=\"{TABLE_CONTAINER_TYPE}\" data-slidra-cols=\"{cols_attr}\" data-slidra-rows=\"{rows_attr}\"{header_attr} data-slidra-theme=\"{}\"{transform_attr}>{source_markup}{cells_markup}</g>",
...
fn render_cell_markup(layout: &CellLayout) -> String {
    ...
    let span_attr = if cell.row_span > 1 || cell.col_span > 1 {
        format!(" data-slidra-span=\"{},{}\"", cell.row_span, cell.col_span)
    } else {
        String::new()
    };
    let repeat_attr = if cell.repeat {
        " data-slidra-repeat=\"row\" display=\"none\""
    } else {
        ""
    };
    let generated_attr = if cell.generated {
        " data-slidra-generated=\"1\""
    } else {
        ""
    };
    let align_attr = if cell.align.as_str() != "left" {
        format!(" data-slidra-align=\"{}\"", cell.align.as_str())
    } else {
        String::new()
    };
    ...
    format!(
        "<g data-slidra-cell=\"{},{}\"{span_attr}{repeat_attr}{generated_attr}{align_attr}{cell_transform}>{rect}{text}</g>",
        cell.row, cell.col,
    )
}
...
mod tests {
    ...
    fn renders_container_with_cols_rows_and_theme() {
        ...
        assert!(markup.starts_with("<g id=\"el-1\" data-slidra-type=\"table\""));
        assert!(markup.contains("data-slidra-cols=\"100\""));
        assert!(markup.contains("data-slidra-theme=\"dark\""));
        assert!(!markup.contains("data-slidra-header"));
    }
    ...
    fn header_attr_present_only_when_header_true() {
        ...
        model.header = true;
        let markup = render_table_markup(&model, "el-1", None, &fonts_with_default()).unwrap();
        assert!(markup.contains("data-slidra-header=\"1\""));
    }
    ...
    fn source_markup_present_only_when_bound() {
        ...
        let markup = render_table_markup(&model, "el-1", None, &fonts_with_default()).unwrap();
        assert!(markup.contains("<slidra:source"));
    ...
    fn cells_are_sorted_row_then_col_regardless_of_input_order() {
        ...
        assert!(pos("data-slidra-cell=\"0,0\"") < pos("data-slidra-cell=\"0,1\""));
        assert!(pos("data-slidra-cell=\"0,1\"") < pos("data-slidra-cell=\"1,0\""));
        assert!(pos("data-slidra-cell=\"1,0\"") < pos("data-slidra-cell=\"1,1\""));
    }
    ...
    fn span_attr_present_only_when_spanning() {
        ...
        let markup = render_table_markup(&model, "el-1", None, &fonts_with_default()).unwrap();
        assert!(markup.contains("data-slidra-span=\"1,2\""));
    }
    ...
    fn repeat_and_generated_attrs() {
        ...
        let markup = render_table_markup(&model, "el-1", None, &fonts_with_default()).unwrap();
        assert!(markup.contains("data-slidra-repeat=\"row\" display=\"none\""));
        assert!(markup.contains("data-slidra-generated=\"1\""));
    }
    ...
    fn align_attr_omitted_for_left_present_otherwise() {
        ...
        let markup = render_table_markup(&model, "el-1", None, &fonts_with_default()).unwrap();
        assert!(markup.contains("data-slidra-align=\"center\""));
    }
}
