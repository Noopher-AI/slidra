...
//! 6.2): `comotion` the library's exported functions, fed fixtures whose
...
use comotion::container::{pack_directory, unpack_container};
use comotion::geometry::bbox::{
    ElementBoundsOptions, TextBoundsContext, element_bounds, format_bbox, path_bounds,
    primitive_bounds,
};
use comotion::geometry::transform::IDENTITY;
use comotion::media_format::{MediaKind, detect_media_format};
use comotion::slide::format::{SlideElement, SlideElementKind, SlidePrimitive, TextAlign};
use comotion::slide::scan::{ScannedNode, scan_document};
use comotion::slide::table_grid::TableGrid;
use comotion::svgnum::format_svg_number;
use comotion::text::font::{DEFAULT_FONT_BYTES, FontMetrics, parse_font};
...
/// run end-to-end through the real CLI (`comotion textbox add`) and
...
fn assert_formatted_rect_eq(
    actual: &comotion::geometry::bbox::FormattedBbox,
    expected: &FormattedRectCase,
    label: &str,
) {
...
use comotion::element::arrange::{self, AlignDirection, DistributeAxis};
use comotion::element::clipboard::{self, ClipboardPayload};
use comotion::element::edit::{
    self, InsertElementInput, InsertElementKind, OrderDirection, ResizeAnchor,
};
use comotion::element::group;
use comotion::element::text::{self, AddTextBoxInput, TextStyleUpdate};
use comotion::slide::comments::{self, SlideComment};
use comotion::text::list::ListKind;
