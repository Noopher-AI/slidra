//! CLI-boundary tests: `comotion`'s argv -> (stdout, stderr, exit code)
//! contract (plan section 6.2, boundary 1). Runs the REAL compiled binary
//! via `CARGO_BIN_EXE_comotion` (proving these commands are actually
...
use comotion::commands;

fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is `<repo>/crates/comotion`.
...
impl Fixture {
    ...
    fn run_rust(&self, args: &[&str]) -> Output {
        Command::new(rust_bin())
            .args(args)
            .env("COMOTION_HOME", &self.home)
            .output()
            .expect("compiled comotion binary must run")
    }
}
...
fn concurrent_effect_adds_on_one_slide_are_serialised_by_the_presentation_lock() {
    ...
    let children: Vec<std::process::Child> = (0..PARALLEL)
        .map(|_| {
            Command::new(rust_bin())
                ...
                .env("COMOTION_HOME", &fixture.home)
                .stdout(std::process::Stdio::piped())
                .stderr(std::process::Stdio::piped())
                .spawn()
                .expect("compiled comotion binary must spawn")
        })
        .collect();
    ...
    assert!(
        !String::from_utf8_lossy(&ls.stdout).contains(".comotion.lock"),
        "{}",
...
fn version_flag_is_answered_by_rust_not_node() {
    ...
    assert_eq!(stdout, format!("comotion {}\n", env!("CARGO_PKG_VERSION")));
...
fn no_takeover_table_command_ever_invokes_node() {
    ...
    let empty_path_dir =
        env::temp_dir().join(format!("comotion-empty-path-{}", std::process::id()));
...
fn unknown_command_never_falls_back_to_node() {
    ...
    let empty_path_dir = env::temp_dir().join(format!(
        "comotion-empty-path-unknown-command-{}",
        std::process::id()
    ));
    fs::create_dir_all(&empty_path_dir).unwrap();
...
/// `comotion::base64::encode` (this file has no dependency on that
...
/// Acceptance criterion A3: `comotion asset import <id> <path>` works from
...
fn dash_prefixed_presentation_id_works_for_every_argv_toolkit() {
    ...
    assert_eq!(
        ...
        serde_json::json!(comotion::base64::encode(&real_bytes)),
