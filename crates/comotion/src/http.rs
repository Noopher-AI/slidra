...
pub fn download_source(url: &str) -> CoMotionResult<Vec<u8>> {
    match ureq::get(url).call() {
        Ok(response) => {
            ...
            response
                ...
                .map_err(|_| CoMotionError::invalid(format!("無法下載來源：{url}")))?;
            Ok(bytes)
        }
        Err(ureq::Error::Status(status, _response)) => Err(CoMotionError::invalid(format!(
            "無法下載來源，伺服器回應 {status}：{url}"
        ))),
        Err(ureq::Error::Transport(_)) => {
            Err(CoMotionError::invalid(format!("無法下載來源：{url}")))
        }
    }
}
