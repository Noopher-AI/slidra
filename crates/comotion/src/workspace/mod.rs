//! Workspace read AND write paths. Originally covered only the
//! READ half of the original engine's workspace module (home-dir resolution, the
//! registry read, id-to-workDir lookup) plus its project-json and
//! virtual-fs logic in the `project`/`virtual_fs` submodules — this crate's
//! `registry` submodule now also writes `projects.json` (`new`/`open`/`pack`
//! need to register/update a presentation), and `write.rs` holds the
//! content-write doors (`assertSlidePathListed`, `write_presentation_file`
//! and friends, plus `require_slide` and the per-presentation clipboard file
//! I/O). Container packing/unpacking lives in `container.rs`; format-version
//! migration lives in `migrate.rs`. `list_presentation_entries` (`asset
//! import`'s conflict-free-filename scan) is added directly to this file, and a
...
/// Resolves `COMOTION_HOME`, defaulting to `~/.comotion`. Read fresh on
...
/// as `process.env.COMOTION_HOME` is read fresh on every TS call.
///
/// `env::var_os` (not `env::var`) is used deliberately: TS's `??` operator
/// only falls back on `undefined`/`null`, so `COMOTION_HOME=""` is a valid
...
pub fn resolve_home() -> PathBuf {
    match std::env::var_os("COMOTION_HOME") {
        Some(value) => PathBuf::from(value),
        None => home_dir().join(".comotion"),
    }
}

/// Best-effort `$HOME` lookup for the `COMOTION_HOME`-unset fallback path.
///
...
/// a fallback that only matters when both `COMOTION_HOME` and `HOME` are
...
pub mod registry {
    ...
    /// about lost updates: two `comotion open` runs racing each other each
    ...
    /// another `comotion` command — that would deadlock against this same
    /// lock.
    ///
    ...
    /// Serializes access to the `COMOTION_HOME` env var across this
    ...
    /// race. Every test in this crate that sets `COMOTION_HOME` (directly,
    ...
    mod tests {
        ...
        #[test]
        fn comotion_home_unset_vs_empty_string() {
            let _guard = ENV_LOCK.lock().unwrap();
            let previous = std::env::var_os("COMOTION_HOME");
            unsafe {
            ...
            match previous {
                Some(value) => unsafe { std::env::set_var("COMOTION_HOME", value) },
                None => unsafe { std::env::remove_var("COMOTION_HOME") },
            }
        }
...
mod write_path_tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "comotion-test-workspace-write-{label}-{}",
            crate::id::random_hex_suffix()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        dir
