...
//! - `resolve_virtual_file_path(work_dir, virtual_path) -> CoMotionResult<PathBuf>`
//! - `read_virtual_file(work_dir, virtual_path) -> CoMotionResult<String>` (strict UTF-8)
//! - `read_virtual_file_bytes(work_dir, virtual_path) -> CoMotionResult<Vec<u8>>` (raw bytes)
//!
...
fn build_virtual_tree(work_dir: &Path) -> CoMotionResult<VirtualNode> {
...
pub fn list_virtual_entries(work_dir: &Path, virtual_path: &str) -> CoMotionResult<Vec<String>> {
...
pub fn resolve_virtual_file_path(work_dir: &Path, virtual_path: &str) -> CoMotionResult<PathBuf> {
    let root = build_virtual_tree(work_dir)?;
    let segments = split_virtual_path(virtual_path);
    match navigate(&root, &segments) {
...
pub fn read_virtual_file(work_dir: &Path, virtual_path: &str) -> CoMotionResult<String> {
    let real_path = resolve_virtual_file_path(work_dir, virtual_path)?;
    let bytes = std::fs::read(&real_path)
        .map_err(|_| CoMotionError::invalid(format!("讀取檔案時發生錯誤：{virtual_path}")))?;
    String::from_utf8(bytes)
        .map_err(|_| CoMotionError::invalid(format!("{virtual_path} 是二進位資產，無法以文字讀取")))
}
...
pub fn read_virtual_file_bytes(work_dir: &Path, virtual_path: &str) -> CoMotionResult<Vec<u8>> {
    let real_path = resolve_virtual_file_path(work_dir, virtual_path)?;
