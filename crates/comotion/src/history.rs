...
//!
//! Storage lives at `<COMOTION_HOME>/history/<presentation-id>/`, a sibling
...
//! `COMOTION_HOME` plus the work directory themselves via `crate::workspace`
...
//! site. A CLI command handler for `comotion undo`/`redo` needs only the
...
pub(crate) fn discard_snapshot_entries(id: &str, entries: &[HistoryEntry]) -> CoMotionResult<()> {
    let home = workspace::resolve_home();
