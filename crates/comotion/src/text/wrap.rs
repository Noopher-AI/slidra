...
fn assert_width(width: f64) -> CoMotionResult<()> {
...
pub fn wrap_text(text: &str, options: &WrapOptions) -> CoMotionResult<WrappedText> {
    assert_width(options.width)?;
    if text.contains('\r') {
        return Err(CoMotionError::invalid("文字內容不接受 \\r，硬換行請用 \\n"));
    }
