...
//!
//! 1. Explicit `if (...) throw new CoMotionError(...)` guards at specific
...
//!    as explicit `CoMotionError` returns, at the same points, with the same
//!    messages.
...
//!    would throw an uncaught, non-`CoMotionError` `RangeError` if a table
...
/// relative to this crate's `Cargo.toml` (`crates/comotion/`): `../../`
...
fn corrupt() -> CoMotionError {
    CoMotionError::invalid("字型檔案無效或已損毀")
}
...
fn require_table(tables: &HashMap<String, TableRecord>, tag: &str) -> CoMotionResult<TableRecord> {
    tables
        .get(tag)
        .copied()
...
fn parse_coverage(bytes: &[u8], offset: usize) -> CoMotionResult<HashMap<u16, u16>> {
...
fn parse_class_def(bytes: &[u8], offset: usize) -> CoMotionResult<HashMap<u16, u16>> {
    let format = u16_at(bytes, offset);
    let mut map = HashMap::new();
    if format == 1 {
...
fn parse_pair_kerning(bytes: &[u8], gpos: &TableRecord) -> CoMotionResult<Vec<PairPosSubtable>> {
...
fn parse_cmap(bytes: &[u8], cmap: TableRecord) -> CoMotionResult<CmapKind> {
    if cmap.length < 4 {
        return Err(corrupt());
    }
    ...
    Err(CoMotionError::invalid("字型的 cmap 格式不支援"))
}

fn build_format12(bytes: &[u8], offset: usize) -> CoMotionResult<CmapKind> {
...
pub fn parse_font(bytes: &[u8]) -> CoMotionResult<ParsedFont> {
    if bytes.len() < 12 {
        return Err(corrupt());
    }
