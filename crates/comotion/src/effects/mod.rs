...
/// The four required attributes of a `<comot:effect>`, read as raw strings
...
/// string.
pub fn assert_legal_seconds(value: f64, label: &str, attr_name: &str) -> CoMotionResult<()> {
    if !value.is_finite() || value < 0.0 {
        return Err(CoMotionError::invalid(format!(
...
pub fn derive_steps(effects: &[Effect]) -> CoMotionResult<Vec<Step>> {
