...
//! 4. Syntax it cannot make sense of returns `CoMotionError::invalid`. It
//!    never guesses.
//!
...
fn scan_attributes(
    ...
    from: usize,
    to: usize,
) -> CoMotionResult<Vec<ScannedAttribute>> {
    ...
    while i < to {
        ...
        if name.is_empty() {
            return Err(CoMotionError::invalid("屬性語法錯誤：無法解析屬性名稱"));
        }
...
fn read_open_tag(svg: &str, tracker: &mut Utf16Tracker, start: usize) -> CoMotionResult<OpenTag> {
    let mut j = start + 1;
...
pub fn scan_document(svg: &str) -> CoMotionResult<Vec<ScannedNode>> {
