...
//! `require_svg_root` returns a `CoMotionError` when the document has no
...
//! fallible (`CoMotionResult<PageStyle>`).
...
/// the root `<svg>`'s own `style` attribute.
...
/// `comotion slide style set`'s update input. `None` leaves that axis
...
/// Sets/clears the root `<svg>`'s Page style (`comotion slide style set`).
