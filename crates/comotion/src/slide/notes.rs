...
const NOTES_TAG: &str = "comot:notes";
const METADATA_TAG: &str = "metadata";
/// Same namespace URI `<comot:effects>`/`<comot:transition>` bind — an
/// unbound `comot:` prefix is a fatal XML parse error, not a tolerated one.
pub const NOTES_NS: &str = "https://co-motion.dev/ns";
...
/// Sets a slide's speaker notes to `text` (`comotion slide notes set`).
...
/// any existing `<comot:effects>`/`<comot:transition>` untouched); replaces
/// the content of an existing `<comot:notes>` otherwise. An empty `text` is a
/// legal way to clear the notes — it writes `<comot:notes></comot:notes>`,
/// never removes the element and never errors.
pub fn set_slide_notes(svg_content: &str, text: &str) -> CoMotionResult<String> {
    ...
    let Some(metadata) = metadata else {
        let markup = format!(
            "<{METADATA_TAG}><{NOTES_TAG} xmlns:comot=\"{NOTES_NS}\">{escaped}</{NOTES_TAG}></{METADATA_TAG}>"
        );
    ...
    let Some(notes) = notes else {
        let markup = format!("<{NOTES_TAG} xmlns:comot=\"{NOTES_NS}\">{escaped}</{NOTES_TAG}>");
    ...
    // A `<comot:notes>` written without this namespace (or with the wrong
    ...
    // created ones.
    let has_correct_ns =
        attribute_of(notes, "xmlns:comot").map(|a| a.value.as_str()) == Some(NOTES_NS);
    if !has_correct_ns {
        let markup = format!("<{NOTES_TAG} xmlns:comot=\"{NOTES_NS}\">{escaped}");
...
mod tests {
    ...
    fn empty_text_writes_empty_element_not_error() {
        ...
        assert!(
            updated
                .contains("<comot:notes xmlns:comot=\"https://co-motion.dev/ns\"></comot:notes>")
        );
    }
    ...
    fn replaces_existing_notes_content_leaving_effects_untouched() {
        let svg = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1280 720\"><metadata><comot:effects xmlns:comot=\"https://co-motion.dev/ns\"></comot:effects><comot:notes xmlns:comot=\"https://co-motion.dev/ns\">old</comot:notes></metadata></svg>\n";
        let updated = set_slide_notes(svg, "new").unwrap();
        assert!(
            updated.contains(
                "<comot:effects xmlns:comot=\"https://co-motion.dev/ns\"></comot:effects>"
            )
        );
        assert!(
            updated.contains(
                "<comot:notes xmlns:comot=\"https://co-motion.dev/ns\">new</comot:notes>"
            )
        );
    ...
    fn inserts_notes_at_the_start_of_existing_metadata_before_effects() {
        ...
        // same insertion point for a sibling `<comot:*>` element).
        let svg = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1280 720\"><metadata><comot:effects xmlns:comot=\"https://co-motion.dev/ns\"></comot:effects></metadata></svg>\n";
        let updated = set_slide_notes(svg, "note").unwrap();
        assert!(updated.contains(
            "<comot:notes xmlns:comot=\"https://co-motion.dev/ns\">note</comot:notes><comot:effects xmlns:comot=\"https://co-motion.dev/ns\"></comot:effects>"
        ));
    }
    ...
    fn cjk_content_before_insertion_point_uses_correct_offset() {
        ...
        assert!(
            updated.contains(
                "<comot:notes xmlns:comot=\"https://co-motion.dev/ns\">note</comot:notes>"
            )
        );
    }
}
