...
/// (`comotion text set`). Does NOT call `assert_slide_compliant` (see
/// module doc comment).
...
/// (`comotion textbox width`). The text itself is unchanged; only
...
/// `comotion textbox align`: sets a text box's `data-comot-text-align` and
...
/// `comotion text style set`'s mutation primitive: sets or clears
...
/// `comotion text list set`'s mutation primitive: sets one paragraph's
...
/// `data-comot-text-width`/`-height` and the translate, the content
...
pub(crate) fn build_text_box_markup(
    ...
    let name_attr = name
        .map(|n| format!(" data-comot-name=\"{}\"", escape_xml_attr(n)))
        .unwrap_or_default();
...
/// `data-comot-text-width`/`-height` and the translate, wrapping a
...
mod tests {
    ...
    fn text_set_on_a_text_box_rewraps_and_keeps_its_list_state() {
        ...
        assert!(updated.contains(r#"data-comot-list="bullet""#), "{updated}");
        assert_eq!(
            updated.matches("data-comot-list-marker").count(),
            1,
            "{updated}"
        );
    ...
    fn text_set_full_replace_keeps_the_list_and_extends_it_to_new_paragraphs() {
        ...
        assert!(
            updated.contains(r#"data-comot-list="bullet bullet bullet""#),
            "{updated}"
        );
    ...
    fn text_set_full_replace_on_a_box_without_a_list_writes_no_list_attribute() {
        ...
        let updated = replace_element_text(&svg, "tb", "new text", &font_book(), false).unwrap();
        assert!(!updated.contains("data-comot-list"), "{updated}");
    }
