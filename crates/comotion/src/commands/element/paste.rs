//! `comotion element paste` argv layer.
...
fn try_run(args: &[String]) -> CoMotionResult<CommandResult> {
    ...
    let svg_from_file = match svg_file {
        Some(path) => Some(
            std::fs::read_to_string(path)
                .map_err(|_| CoMotionError::invalid(format!("找不到來源檔案：{path}")))?,
        ),
        None => None,
    };
    ...
    let payload = match svg_from_file {
        Some(svg) => clipboard::parse_clipboard_svg(&svg).ok_or_else(|| {
            CoMotionError::invalid("剪貼簿內容不是合法的 comotion 元素剪貼簿格式")
        })?,
        None => {
            let raw = write::read_clipboard_file(&id)?;
            serde_json::from_str(&raw).map_err(|_| CoMotionError::invalid("剪貼簿資料已損毀"))?
        }
    };
