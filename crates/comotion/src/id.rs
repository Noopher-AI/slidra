...
//! documented security property (ADR-0004). `COMOTION_ID_SEED` is therefore
...
thread_local! {
    ...
    // `COMOTION_ID_SEED` differs from the last call, so that switching the
...
/// Reads `n` random bytes, or — when `COMOTION_ID_SEED` is set — derives
...
fn random_bytes(n: usize) -> Vec<u8> {
    match std::env::var("COMOTION_ID_SEED") {
...
mod tests {
    ...
    fn generate_opaque_id_is_deterministic_under_seed() {
        unsafe {
            std::env::set_var("COMOTION_ID_SEED", "golden-fixture-seed-a");
        }
        ...
        unsafe {
            std::env::remove_var("COMOTION_ID_SEED");
        }
        assert_eq!(a1.len(), 12);
        assert_eq!(a2.len(), 12);
        assert_ne!(a1, a2);
