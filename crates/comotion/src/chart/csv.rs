...
fn parse_rows(text: &str) -> CoMotionResult<Vec<Vec<String>>> {
    ...
    if in_quotes {
        return Err(CoMotionError::invalid("CSV 語法錯誤：引號未封閉"));
    }
...
pub fn parse_chart_csv(text: &str) -> CoMotionResult<ParsedChartCsv> {
    let rows: Vec<Vec<String>> = parse_rows(strip_bom(text))?
        .into_iter()
        .filter(|row| !(row.len() == 1 && row[0].is_empty()))
        .collect();
    if rows.is_empty() {
        return Err(CoMotionError::invalid("CSV 沒有任何資料列"));
    }
    let header = &rows[0];
    ...
    if series_names.is_empty() {
        return Err(CoMotionError::invalid("CSV 標頭沒有任何系列欄位"));
    }
