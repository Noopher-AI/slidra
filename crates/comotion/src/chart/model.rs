//! The `<comot:chart>` data model, ported from `packages/core/src/chart/model.ts`
...
//! element_id) -> CoMotionResult<&ScannedNode>`); every caller here already
...
/// `<comot:chart>`). Once a value is one of this module's enums, Rust's
...
/// Every invariant a `<comot:chart>` must hold for `render_chart_svg` to
...
pub fn validate_chart_model(model: &ChartModel) -> CoMotionResult<()> {
    ...
    if model.categories.iter().any(|category| category.is_empty()) {
        return Err(CoMotionError::invalid("類別名稱不可為空字串"));
    }
    // `serialize_chart_data` joins categories with "," into
    // <comot:categories values="…">; a category name that itself contains
    ...
    if model.stacked {
        ...
        if model.axes != ChartAxesMode::Single {
            return Err(CoMotionError::invalid("堆疊圖表必須是 axes=single"));
        }
    }
...
pub fn require_chart_container<'a>(
    ...
    if attribute_value(found, "data-comot-type").as_deref() != Some(CHART_CONTAINER_TYPE) {
        return Err(CoMotionError::invalid(format!(
            "元素 {element_id} 不是圖表"
        )));
    }
...
fn require_data_node<'a>(
    ...
    container
        ...
        .ok_or_else(|| CoMotionError::invalid(format!("元素 {element_id} 缺少 <comot:chart>")))
}

fn read_number_attr(node: &ScannedNode, name: &str, element_id: &str) -> CoMotionResult<f64> {
    let raw = attribute_of(node, name).ok_or_else(|| {
        CoMotionError::invalid(format!(
            "元素 {element_id} 的 <comot:chart> 缺少屬性：{name}"
        ))
    })?;
    let value = crate::argv::parse_js_number(&raw.value).unwrap_or(f64::NAN);
    if !value.is_finite() {
        return Err(CoMotionError::invalid(format!(
            "元素 {element_id} 的 <comot:chart> 屬性 {name} 不是有限數字：{}",
            raw.value
        )));
    }
    Ok(value)
...
fn read_required_attr(node: &ScannedNode, name: &str, element_id: &str) -> CoMotionResult<String> {
...
fn parse_values(raw: &str, element_id: &str, series_name: &str) -> CoMotionResult<Vec<f64>> {
    raw.split(',')
        .enumerate()
        .map(|(index, token)| {
...
/// Reads `element_id`'s `<comot:chart>` back into a `ChartModel`.
...
pub fn read_chart_model(svg_content: &str, element_id: &str) -> CoMotionResult<ChartModel> {
    ...
    let series_nodes: Vec<&ScannedNode> = chart_node
        .children
        .iter()
        .filter(|child| child.tag == "comot:series")
        .collect();
    let categories_nodes: Vec<&ScannedNode> = chart_node
        .children
        .iter()
        .filter(|child| child.tag == "comot:categories")
        .collect();
    if series_nodes.is_empty() {
        return Err(CoMotionError::invalid(format!(
            "元素 {element_id} 的圖表沒有任何 <comot:series>"
        )));
    }
    if categories_nodes.len() != 1 {
        return Err(CoMotionError::invalid(format!(
            "元素 {element_id} 的圖表必須恰好有一個 <comot:categories>"
        )));
    }
    if let Some(other) = chart_node
        .children
        .iter()
        .find(|child| child.tag != "comot:series" && child.tag != "comot:categories")
    {
        return Err(CoMotionError::invalid(format!(
            "元素 {element_id} 的 <comot:chart> 含有未知子元素：<{}>",
            other.tag
        )));
    }
...
/// Serializes `model` back into `<comot:chart>…</comot:chart>`, attribute
/// order fixed (plan §4.1).
pub fn serialize_chart_data(model: &ChartModel) -> String {
    let attrs = [
        format!("xmlns:comot=\"{CHART_NS}\""),
    ...
    format!("<comot:chart {attrs}>{series_markup}{categories_markup}</comot:chart>")
}
...
mod tests {
    ...
    fn serialize_chart_data_matches_fixed_attribute_order() {
        ...
        assert!(markup.starts_with(&format!(
            "<comot:chart xmlns:comot=\"{CHART_NS}\" type=\"bar\" stacked=\"false\" axes=\"single\" palette=\"brand\" legend=\"bottom\" grid=\"true\" labels=\"true\" x-title=\"\" y-title=\"\" width=\"100\" height=\"100\">"
        )));
        assert!(
            markup.contains(r##"<comot:series name="S1" values="1,2" axis="left" color="#fff"/>"##)
        );
        assert!(markup.contains(r#"<comot:categories values="C1,C2"/>"#));
    }

    #[test]
    fn require_chart_container_reports_missing_element() {
        let svg = r#"<svg viewBox="0 0 10 10"><g id="other" data-comot-type="chart"/></svg>"#;
    ...
    fn read_chart_model_round_trips_serialize_output() {
        ...
        let markup = serialize_chart_data(&model);
        let svg = format!(
            r#"<svg viewBox="0 0 100 100"><g id="e1" data-comot-type="chart">{markup}<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"/></g></svg>"#
        );
