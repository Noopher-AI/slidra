...
//! rather than handed to `clap` for the whole argv: `comotion`'s existing
...
use comotion::node_entry;
use comotion::result::{self, Renderer};
use comotion::workspace::lock::PresentationLock;
...
fn dispatch(argv: Vec<OsString>) -> i32 {
    let Some(first) = argv.first() else {
        // argv is empty (`comotion`) — there is no coexistence-era fallback
    ...
    if let Some(first_str) = first.to_str() {
        if first_str == "--version" || first_str == "-V" {
            ...
            println!("comotion {}", env!("CARGO_PKG_VERSION"));
            return 0;
        }
...
fn dispatch_legacy_takeover(command: &str, rest: &[OsString]) -> i32 {
    ...
    let (command_result, renderer): (comotion::result::CommandResult, Option<Renderer<'_>>) =
        match command {
...
fn hold_presentation_lock(positional: &[String]) -> Result<Option<PresentationLock>, i32> {
    for candidate in positional.iter().take(3) {
        ...
        let Ok(work_dir) = comotion::workspace::resolve_work_dir(candidate) else {
            continue;
        };
