...
fn read_view_box_exists(svg_content: &str) -> CoMotionResult<()> {
...
fn update_table(
    ...
    mutate: impl FnOnce(TableModel) -> CoMotionResult<TableModel>,
...
fn patch_cell_style(cell: TableCell, attr: &str, value: &str) -> CoMotionResult<TableCell> {
    match attr {
        "align" => {
        ...
        other => Err(CoMotionError::invalid(format!("不支援的樣式屬性：{other}"))),
    }
}
...
fn is_hex_6_strict(value: &str) -> CoMotionResult<()> {
    let bytes = value.as_bytes();
    if bytes.len() == 7 && bytes[0] == b'#' && bytes[1..].iter().all(u8::is_ascii_hexdigit) {
        Ok(())
    } else {
        Err(CoMotionError::invalid(""))
    }
}
...
pub fn set_table_cell_style(
    ...
    update_table(svg_content, slide_path, element_id, fonts, move |model| {
        ...
        if !matched {
            return Err(CoMotionError::invalid("指定範圍內沒有任何儲存格"));
        }
        Ok(TableModel { cells, ..model })
    })
}
...
pub fn merge_table_cells(
    ...
    update_table(svg_content, slide_path, element_id, fonts, move |model| {
        ...
        if row_span_raw.fract() != 0.0
            ...
            || col_span_raw < 1.0
        {
            return Err(CoMotionError::invalid("合併範圍必須是正整數"));
        }
...
pub fn delete_table_row(
    ...
    update_table(svg_content, slide_path, element_id, fonts, move |model| {
        ...
        if is_template_row {
            return Err(CoMotionError::invalid("模板列不可刪除，請先解除綁定"));
        }
...
fn substitute_template(
    ...
    loop {
        ...
        if name.is_empty() {
            return Err(CoMotionError::invalid("模板格的 {{ }} 不可為空名稱"));
        }
...
fn expand_template_row(
    ...
    template_row: usize,
    csv: &ParsedTableCsv,
) -> CoMotionResult<(Vec<TableCell>, usize)> {
        ...
        let resolved_template_row = match template_row {
            Some(requested) => {
                ...
                if model.header && requested == 0 {
                    return Err(CoMotionError::invalid("表頭列不能當模板列"));
                }
                requested
            }
            None => {
                if model.header && row_count_after_strip == 1 {
                    return Err(CoMotionError::invalid("表格只有表頭列，沒有可當模板的列"));
                }
...
pub fn refresh_table_source(
    ...
    update_table(svg_content, slide_path, element_id, fonts, move |model| {
        ...
        let template_row = stripped
            ...
            .map(|cell| cell.row)
            .ok_or_else(|| {
                CoMotionError::invalid(format!("表格 {element_id_for_error} 沒有模板列"))
            })?;
...
mod tests {
    ...
    fn create_table_element_builds_grid_and_transform() {
        ...
        assert!(svg.contains(r#"id="t1" data-comot-type="table""#));
        assert!(svg.contains("transform=\"translate(10 20)\""));
        assert!(svg.contains(r#"data-comot-cell="0,0""#));
        assert!(svg.contains(r#"data-comot-cell="1,1""#));
    }
