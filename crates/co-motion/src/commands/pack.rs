...
mod tests {
    ...
    fn unknown_id_is_not_found() {
        ...
        let result = run(&["nope".to_string(), "/tmp/out.comot".to_string()]);
        assert!(!result.ok);
        assert_eq!(
            result.failure_kind,
    ...
    fn packing_to_the_same_path_updates_saved_at() {
        ...
        let output = temp_dir("same-path-output").join("out.comot");
    ...
    fn packing_to_a_different_path_leaves_saved_at_untouched() {
        ...
        registry_map.insert(
            ...
            RegistryEntry {
                work_dir: work.clone(),
                source_path: Some(PathBuf::from("/some/other/path.comot")),
                saved_at: Some(0.0),
            },
        );
        workspace::registry::write_registry(&home, &registry_map).unwrap();

        let output = temp_dir("diff-path-output").join("out.comot");
        let result = run(&["id1".to_string(), output.to_string_lossy().into_owned()]);
        assert!(result.ok);
