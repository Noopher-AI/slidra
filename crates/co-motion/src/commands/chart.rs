...
fn result_of(
    outcome: Result<(String, String, serde_json::Value, String), CoMotionError>,
) -> CommandResult {
    match outcome {
        ...
        ChartCommand::DataSet {
            ...
            let (categories, series) = if let Some(csv_value) = csv {
                let text = if csv_value == "-" {
                    stdin_csv
                        .clone()
                        .ok_or_else(|| CoMotionError::invalid("--csv - 只能從命令列使用"))?
                } else {
                    std::fs::read_to_string(&csv_value).map_err(|_| {
                        CoMotionError::not_found(format!("找不到 CSV 檔案：{csv_value}"))
                    })?
                };
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "co-motion-test-commands-chart-{label}-{}",
    ...
    fn create_then_data_set_writes_expected_markup() {
        ...
        assert!(svg.contains(r#"<comot:categories values="Q1,Q2"/>"#));
