//! `co-motion effect add / remove / move / set / list`. Ported from
...
/// An argv-parsing failure (`packages/cli/src/argv.ts`'s own `CoMotionError`
...
fn optional_flag(args: &[String], flag: &str) -> CoMotionResult<Option<String>> {
    let Some(index) = args.iter().position(|a| a == flag) else {
        return Ok(None);
    };
    match args.get(index + 1) {
...
fn optional_number_flag(args: &[String], flag: &str) -> CoMotionResult<Option<f64>> {
...
/// `Vec<String>`.
fn require_id_list(args: &[String], index: usize, command: &str) -> CoMotionResult<Vec<String>> {
...
/// `Vec<i64>`.
fn require_index_list(args: &[String], index: usize, command: &str) -> CoMotionResult<Vec<i64>> {
...
/// set`).
fn require_index(args: &[String], index: usize, command: &str) -> CoMotionResult<i64> {
...
fn read_slide_for_write(id: &str, slide_path: &str) -> CoMotionResult<String> {
...
fn list_data(id: &str, slide_path: &str) -> CoMotionResult<(usize, Value)> {
    let work_dir = workspace::resolve_work_dir(id)?;
    workspace::virtual_fs::resolve_virtual_file_path(&work_dir, slide_path)?;
    workspace::write::assert_slide_path_listed(&work_dir, slide_path)?;
