...
mod style_set {
    ...
    fn parse_range(raw: &str) -> CoMotionResult<(usize, usize)> {
        let (left, right) = raw.split_once(':').ok_or_else(|| {
            CoMotionError::invalid(format!("--range 格式錯誤，必須是 數字:數字：{raw}"))
        })?;
...
mod list_set {
    ...
    fn parse_kind(raw: &str) -> CoMotionResult<ListKind> {
        match raw {
    ...
    fn parse_paragraph(raw: &str) -> CoMotionResult<usize> {
        ...
        raw.parse()
            .map_err(|_| CoMotionError::invalid(format!("--paragraph 不是合法的非負整數：{raw}")))
    }
...
mod tests {
    ...
    fn text_style_set_and_text_list_set_via_the_cli_layer() {
        ...
        fixture.write_slide(&slide(
            r#"<g id="tb" data-comot-text-width="500"><text font-family="Noto Sans TC" font-size="16" xml:space="preserve"><tspan x="0" y="0">hello world</tspan></text></g>"#,
        ));
        ...
        assert!(fixture.read_slide().contains("data-comot-list-marker"));
    }
    ...
    fn text_list_set_none_to_none_occupies_no_undo_step() {
        ...
        let original = slide(
            r#"<g id="tb" data-comot-text-width="500"><text font-family="Noto Sans TC" font-size="16" xml:space="preserve"><tspan x="0" y="0">hi</tspan></text></g>"#,
        );
