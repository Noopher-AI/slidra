...
fn parse_align(
    raw: &str,
    command: &str,
) -> crate::errors::CoMotionResult<crate::slide::format::TextAlign> {
...
mod add {
    ...
    use crate::commands::argv::{
        optional_flag, optional_number_flag, require_number_flag, require_positional,
    };
    ...
    fn try_run(args: &[String]) -> CoMotionResult<CommandResult> {
        let id = require_positional(args, 0, "textbox add", "presentation-id")?.to_string();
...
mod width {
    use super::*;
    use crate::commands::argv::{require_positional, require_trailing_force_flag};
    ...
    fn try_run(args: &[String]) -> CoMotionResult<CommandResult> {
        let id = require_positional(args, 0, "textbox width", "presentation-id")?.to_string();
...
mod align {
    ...
    fn try_run(args: &[String]) -> CoMotionResult<CommandResult> {
        let id = require_positional(args, 0, "textbox align", "presentation-id")?.to_string();
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "co-motion-test-cmd-textbox-{label}-{}",
    ...
    fn textbox_add_width_and_align_via_the_cli_layer() {
        ...
        assert!(
            fixture
                .read_slide()
                .contains(r#"data-comot-text-width="150""#)
        );
        ...
        assert!(
            fixture
                .read_slide()
                .contains(r#"data-comot-text-align="center""#)
        );
    ...
    fn textbox_align_rejects_a_value_outside_the_fixed_set() {
        ...
        fixture.write_slide(&slide(
            r#"<g id="tb" data-comot-text-width="300"><text font-family="Noto Sans TC" font-size="24" xml:space="preserve"><tspan x="0" y="0">hi</tspan></text></g>"#,
        ));
