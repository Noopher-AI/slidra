//! `co-motion validate` — the deterministic design checker (#303,
...
pub struct SlideFacts {
    ...
    /// `<comot:transition>` present in `<metadata>`.
    pub has_transition: bool,
    /// Number of `<comot:effect family="enter">` entries.
...
pub fn read_slide_facts(svg: &str) -> CoMotionResult<SlideFacts> {
    ...
    if let Some(metadata) = find_child(root, "metadata") {
        if let Some(notes) = find_child(metadata, "comot:notes") {
    ...
    for element in parent.children.iter().filter(|c| c.tag == "g") {
        ...
        if let Some(width) = number_attr(element, "data-comot-text-width") {
            ...
            for (index, span) in spans.iter().enumerate() {
                ...
                let ends_paragraph =
                    attribute_value(span, "data-comot-break").is_some() || index + 1 == spans.len();
                if ends_paragraph {
        ...
        // A bare `<text>` (no `data-comot-text-width`) is decoration — the
...
/// `co-motion validate <id> [slide-path]`.
pub fn validate_presentation(
    id: &str,
    slide_path: Option<&str>,
) -> CoMotionResult<ValidationReport> {
    ...
    let checked = match slide_path {
        Some(target) => {
            let Some(index) = project.slides.iter().position(|p| p == target) else {
                return Err(CoMotionError::not_found(format!("不是投影片：{target}")));
            };
...
mod tests {
    ...
    fn textbox(
        ...
        let spans: String = lines
            .iter()
            .map(|(text, brk)| {
                let attr = if *brk { " data-comot-break=\"1\"" } else { "" };
                format!("<tspan x=\"0\" y=\"0\"{attr}>{text}</tspan>")
            })
            .collect();
        format!(
            "<g id=\"{id}\" transform=\"translate({x} {y})\" data-comot-text-width=\"{width}\"><text font-family=\"Noto Sans TC\" font-size=\"{size}\" fill=\"{fill}\">{spans}</text></g>"
        )
    }
    ...
    /// `comotion-build` leaves behind, so the motion rules stay quiet.
    ...
    fn slide_with_motion(
        ...
        let motion_markup = if motion {
            "<comot:transition xmlns:comot=\"https://co-motion.dev/ns\" enter=\"fade\" enter-duration=\"0.3\"/><comot:effects xmlns:comot=\"https://co-motion.dev/ns\"><comot:effect target=\"el-title\" family=\"enter\" effect=\"fade\" start=\"on-click\" duration=\"0.4\" delay=\"0\"/></comot:effects>"
        } else {
            ""
        };
        format!(
            "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1280 720\"{style}><metadata><comot:notes xmlns:comot=\"https://co-motion.dev/ns\">{notes}</comot:notes>{motion_markup}</metadata>{body}</svg>"
        )
    }
    ...
    fn scrim_is_required_under_body_copy_only_when_the_page_has_a_background_image() {
        let bg = "<g id=\"el-background\" data-comot-name=\"背景圖\" data-comot-role=\"background\" data-comot-lock=\"true\"><image x=\"0\" y=\"0\" width=\"1280\" height=\"720\" href=\"assets/bg.svg\"/></g>";
        let title = textbox(
            "el-title",
            80.0,
            72.0,
