...
/// Appended to the issues `co-motion convert` can actually repair — never to
/// the ones it refuses to touch.
const CONVERT_HINT: &str = "請執行 co-motion convert <簡報識別碼> 轉換成合規格式。";

/// `data-comot-text-width`; see `SlideElement::text_width`'s doc comment.
pub const TEXT_WIDTH_ATTRIBUTE: &str = "data-comot-text-width";
/// `data-comot-text-height` (NOOP-65 決定 C); see `SlideElement::text_height`.
pub const TEXT_HEIGHT_ATTRIBUTE: &str = "data-comot-text-height";
...
pub struct SlideElement {
    pub id: String,
    /// `data-comot-name`; `None` when absent — conversion never invents a
    /// display name.
    pub name: Option<String>,
    /// `data-comot-media`, verbatim (ADR-0005/0009).
    ...
    /// Parsed `data-comot-text-width`; `None` when the element is not a text
    /// box.
    pub text_width: Option<f64>,
    /// Parsed `data-comot-text-height` (NOOP-65 決定 C); `None` when absent.
...
/// Reads a text box container's `data-comot-text-align`, defaulting to
...
pub fn read_text_align(container: &ScannedNode, element_id: &str) -> CoMotionResult<TextAlign> {
...
/// KNOWN GAP, and `table_grid.rs`'s identical caveat for `data-comot-cols`/
/// `-rows`).
fn js_parse_finite(text: &str) -> Option<f64> {
    text.parse::<f64>().ok()
}

/// Parses `data-comot-text-width`/`-height`'s shared validation: a positive
/// finite number, or an explicit error naming the offending raw value.
fn parse_positive_dimension(raw: &str, element_id: &str, attribute: &str) -> CoMotionResult<f64> {
...
fn to_element(element: &ScannedNode, svg: &str) -> CoMotionResult<SlideElement> {
    ...
    } else if attribute_value(element, "data-comot-type").as_deref() == Some(CHART_CONTAINER_TYPE) {
    ...
    Ok(SlideElement {
        id: element_id,
        name: attribute_value(element, "data-comot-name"),
        media: attribute_value(element, "data-comot-media"),
        kind,
...
pub fn parse_slide(svg: &str, slide_path: Option<&str>) -> CoMotionResult<SlideModel> {
...
mod tests {
    ...
    fn invalid_table_shape_missing_grid_attributes() {
        let svg = r#"<svg viewBox="0 0 100 100"><g id="t1" data-comot-type="table"><g data-comot-cell="0,0"><rect/></g></g></svg>"#;
        assert!(has(svg, ComplianceCode::InvalidTableShape));
    }

    #[test]
    fn invalid_chart_shape_missing_the_rendered_svg() {
        let svg = r#"<svg viewBox="0 0 100 100"><g id="c1" data-comot-type="chart"><comot:chart/></g></svg>"#;
    ...
    fn container_with_align(align: Option<&str>) -> ScannedNode {
        let svg = match align {
            Some(value) => format!(r#"<g id="e1" data-comot-text-align="{value}"/>"#),
