...
/// `co-motion slide add`. With `template_path`, the new slide is the
...
/// `co-motion slide delete`. Allowed to bring `slides` down to zero, and
...
/// `co-motion slide duplicate`: inserted directly after the source; every
...
/// `co-motion slide move`: rewrites only `project.json`'s `slides` order —
...
/// `co-motion template add`: registers the new path in `project.json`'s
...
/// `co-motion template list`: the presentation's templates in
...
/// `co-motion template rename`: renames a template's `name` field only —
...
/// `co-motion template delete`: deletes the SVG file and removes its
...
/// `co-motion slide notes set` — slides only, never templates.
...
/// `co-motion slide transition set`. Reads `slide_path`'s current
