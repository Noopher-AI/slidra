//! Normalisation (`co-motion convert`), ported from
//! `packages/core/src/slide/normalise.ts` (full file, 234 lines): wraps bare
//! primitives into `<g>` containers, and moves `id`/`data-comot-name`/
//! `data-comot-media`/`transform` up onto the container (ADR-0012).
//!
...
pub fn normalise_slide_svg(
...
) -> CoMotionResult<NormaliseResult> {
    ...
    let mut mint_id = |used_ids: &mut HashSet<String>| -> CoMotionResult<String> {
        for _ in 0..8 {
            let id = generate_id();
            if id.is_empty() {
                return Err(CoMotionError::invalid("轉換失敗：識別碼產生器回傳空字串"));
            }
...
fn wrap_primitive(
    svg: &str,
    element: &ScannedNode,
    mint_id: &mut dyn FnMut(&mut HashSet<String>) -> CoMotionResult<String>,
...
fn quote(value: &str) -> CoMotionResult<String> {
    if !value.contains('"') {
