...
//! - `resolve_asset_import(input) -> CoMotionResult<ResolvedAssetImport>` — the
//!   media-asset path (no `--as`).
//! - `resolve_data_asset_import(input) -> CoMotionResult<ResolvedDataAssetImport>`
...
pub fn resolve_asset_import(input: ResolveAssetImportInput) -> CoMotionResult<ResolvedAssetImport> {
    let format = detect_media_format(input.bytes).ok_or_else(|| {
        CoMotionError::invalid("不支援的媒體格式：檔案內容不是已知的圖片、影片或音訊格式")
    })?;
...
pub fn resolve_data_asset_import(
...
) -> CoMotionResult<ResolvedDataAssetImport> {
    ...
    let text = std::str::from_utf8(input.bytes)
        .map_err(|_| CoMotionError::invalid("CSV 內容不是合法的 UTF-8 文字"))?;
    if contains_illegal_csv_control_char(text) {
        return Err(CoMotionError::invalid("CSV 內容不是合法的 UTF-8 文字"));
    }
