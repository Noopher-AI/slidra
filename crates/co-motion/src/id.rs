...
//! documented security property (ADR-0004). `CO_MOTION_ID_SEED` is therefore
...
thread_local! {
    ...
    // `CO_MOTION_ID_SEED` differs from the last call, so that switching the
...
/// Reads `n` random bytes, or — when `CO_MOTION_ID_SEED` is set — derives
...
fn random_bytes(n: usize) -> Vec<u8> {
    match std::env::var("CO_MOTION_ID_SEED") {
