...
//!
//! Every `co-motion` invocation that names a presentation holds this lock
...
//! `slides/00N.svg` at once and left a `<comot:effects>` list the next
//! command could not parse.
//!
//! The lock is a file (`.co-motion.lock`) created with `create_new`, which
...
//! into a `.comot` (`container::collect_files` skips it).
...
pub const LOCK_FILE_NAME: &str = ".co-motion.lock";
...
impl PresentationLock {
    ...
    pub fn acquire(work_dir: &Path) -> CoMotionResult<Self> {
                ...
                Err(err) if err.kind() == std::io::ErrorKind::AlreadyExists => {
                    ...
                    if started.elapsed() >= ACQUIRE_TIMEOUT {
                        return Err(CoMotionError::invalid(
                            "簡報正被另一個 co-motion 命令使用中，等待逾時；請稍後再試",
                        ));
                    }
                    std::thread::sleep(RETRY_INTERVAL);
                }
                Err(_) => {
                    return Err(CoMotionError::invalid("無法建立簡報的鎖定檔"));
                }
            }
        }
    }
