...
describe("parseArgv slide transition set ([E2.T11], replacing presentation transition set)", () => {
  ...
  it("presentation transition set no longer parses as its own command — falls through to the unknown-command default, same as any other unregistered top-level name", () => {
    expect(parseArgv(["presentation", "transition", "set", "p1", "fade"])).toEqual({
      name: "presentation",
      input: {},
    });
  });
