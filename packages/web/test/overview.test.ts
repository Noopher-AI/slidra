...
import { mountOverview, mountGridOverview } from "../src/overview.js";
...
// #55's grid view. mountGridOverview deliberately mirrors mountOverview's
// lazy-loading shape (same reasoning as this file's own header comment on
// wrapSlideDocument/slideDirectory) rather than sharing state with it —
// the two consumers (rail, grid) must materialise thumbnails independently
// so a grid mount never eagerly loads what the rail hasn't.
describe("mountGridOverview", () => {
  it("renders one grid-cell per slide with a page number, and no iframe until observed", () => {
    const { controller } = fakeCanvas({
      slides: ["slides/001.svg", "slides/002.svg", "slides/003.svg"],
      currentIndex: 0,
    });

    mountGridOverview(container, controller, vi.fn());

    const cells = container.querySelectorAll("figure.grid-cell");
    expect(cells).toHaveLength(3);
    const numbers = container.querySelectorAll(".grid-number");
    expect(Array.from(numbers).map((el) => el.textContent)).toEqual(["第 1 頁", "第 2 頁", "第 3 頁"]);
    // Lazy loading (#27's rule, reused here): no <li>/<figure> has
    // intersected yet, so no iframe should exist regardless of slide count.
    expect(container.querySelectorAll("iframe.grid-frame")).toHaveLength(0);
  });

  it("marks only the current slide's <figure> with aria-current, and moves the mark when the index changes", () => {
    const { controller, setState } = fakeCanvas({
      slides: ["slides/001.svg", "slides/002.svg"],
      currentIndex: 0,
    });
    ...
    expect(cells()[1].getAttribute("aria-current")).toBe("false");

    setState({ slides: ["slides/001.svg", "slides/002.svg"], currentIndex: 1 });

    expect(cells()[0].getAttribute("aria-current")).toBe("false");
    expect(cells()[1].getAttribute("aria-current")).toBe("true");
  });

  it("clicking a cell's button jumps the canvas to that slide and calls onSelect", () => {
    const { controller, showSlideCalls } = fakeCanvas({
      slides: ["slides/001.svg", "slides/002.svg"],
      currentIndex: 0,
    });
    ...
    mountGridOverview(container, controller, onSelect);

    const buttons = container.querySelectorAll("button.grid-thumb");
    (buttons[1] as HTMLButtonElement).click();
  ...
  it("only materialises a cell's iframe (and fetches its slide) once it is observed to intersect, independently of the rail", async () => {
    const fetchCalls: string[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: string | URL) => {
    ...
    const observer = FakeIntersectionObserver.instances.at(-1)!;
    observer.triggerIntersect(cells[0]);
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(fetchCalls).toEqual(["/api/files/slides/001.svg"]);
    const frame = cells[0].querySelector("iframe.grid-frame") as HTMLIFrameElement;
    expect(frame).not.toBeNull();
    expect(frame.getAttribute("sandbox")).toBe("");
    expect(frame.srcdoc).toContain('data-testid="s1"');
    expect(cells[1].querySelector("iframe")).toBeNull();
  });

  it("destroy() removes the cells from the container and stops responding to further canvas updates", () => {
    const { controller, setState } = fakeCanvas({ slides: ["slides/001.svg"], currentIndex: 0 });
    const overview = mountGridOverview(container, controller, vi.fn());

    expect(container.querySelector("figure.grid-cell")).not.toBeNull();

    overview.destroy();

    expect(container.querySelector("figure.grid-cell")).toBeNull();
    expect(() => setState({ slides: ["slides/001.svg", "slides/002.svg"], currentIndex: 1 })).not.toThrow();
  });
});
