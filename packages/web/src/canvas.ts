...
export interface CanvasController {
  ...
  /** ⌘V, or the ContextBar Paste button (plan §4.3): routes `text` — a comotion elements payload, or plain text with a cell range selected — to the matching command; silent no-op for anything else (including plain text with nothing selected). The window `paste` event's own image-file branch (App.tsx) is untouched and independent of this. */
 ...
 */
/**
 * Textbox mid-edge width drag (decision (b)): no font is fetched
...
export function mountCanvas(container: HTMLElement): CanvasController {
  ...
  /** `+3% / +4%` of the viewBox — `runCommand`'s existing `SELECT_AFTER_COMMAND` entry for `"element duplicate"` selects the new copy on success. */
  ...
  async function renderPlay(
    ...
    try {
      ...
      // A slide whose <comot:transition> is present but malformed
