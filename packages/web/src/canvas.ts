...
import { buildFrame, createPlayMode, type PlayModeDeps } from "./canvas/play-mode.js";
...
export function mountCanvas(container: HTMLElement): CanvasController {
  ...
  /**
   * [E8.T4] Same rehearsal pattern as `gestureDeps`/`runtimeMessageDeps`
   * above, now wired for real: this object is the `PlayModeDeps` [E8.T3]
   * measured, and `createPlayMode` right below turns it into the twelve
   * play-mode handlers `canvas/play-mode.ts` exports. Built here, right
   * after `gestures`, so `runtimeMessageDeps.playback` below can reference
   * `playMode.*` directly — `const` bindings are subject to TDZ where
  ...
  const runtimeMessageDeps: RuntimeMessageDeps = {
    ...
    playback: {
      focusPlayer: playMode.focusPlayer,
      advancePastEnd,
      retreatPastStart,
      exitPreview: playMode.exitPreview,
      exitPlay: playMode.exitPlay,
      next: playMode.next,
      previous: playMode.previous,
    },
    commands: {
      selectAll,
      deleteSelection,
  ...
  async function advancePastEnd(): Promise<void> {
    ...
    if (!(await playMode.playExitTransition(thisGeneration))) return;
    currentIndex += 1;
    notify();
    await playMode.renderPlay(thisGeneration, "first", true);
  }
  ...
  async function retreatPastStart(): Promise<void> {
    ...
    currentIndex -= 1;
    notify();
    await playMode.renderPlay(thisGeneration, "last", true);
  }
  ...
  async function reload(): Promise<void> {
    ...
    if (mode === "play") {
      ...
      await playMode.renderPlay(thisGeneration, "first", false);
    } else {
  ...
  return {
    ...
    play: playMode.play,
    exitPlay: playMode.exitPlay,
    previewEffects: playMode.previewEffects,
    exitPreview: playMode.exitPreview,
    focusPlayer: playMode.focusPlayer,
    stepPlayer: playMode.stepPlayer,
