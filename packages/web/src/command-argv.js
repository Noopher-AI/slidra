...
export const ARGV_ENCODERS = {
    ...
    "slide add": (i) => plain(["slide", "add", str(i.id), ...optFlag("--template", i.templatePath), ...optFlag("--at", i.at)]),
    ...
    "element paste": async (i) => {
        ...
        if (typeof i.svg === "string") {
            return { argv: [...base, "--svg-file", "-"], body: new TextEncoder().encode(i.svg), cleanup: NOOP_CLEANUP };
        }
        return plain([...base, ...optFlag("--svg-file", i.svgFile)]);
    },
    "element insert": (i) => plain([
    ...
    "element distribute": (i) => plain(["element", "distribute", str(i.id), str(i.slidePath), idsToken(i.elementIds), str(i.axis)]),
    "element order": (i) => plain([
        "element", "order", str(i.id), str(i.slidePath), idsToken(i.elementIds), str(i.direction),
        ...boolFlag("--force", i.force),
    ]),
    "element style set": (i) => plain([
        "element", "style", "set", str(i.id), str(i.slidePath), idsToken(i.elementIds), str(i.attr), str(i.value),
        ...boolFlag("--force", i.force),
    ]),
    "slide transition set": (i) => plain([
    ...
    "element name set": (i) => plain(["element", "name", "set", str(i.id), str(i.slidePath), idsToken(i.elementIds), str(i.name)]),
    "effect add": (i) => plain([
    ...
    "effect remove": (i) => {
        const indices = Array.isArray(i.indices) ? i.indices.join(",") : "";
    ...
    "comment add": (i) => plain(["comment", "add", str(i.id), str(i.slidePath), str(i.target), str(i.text), ...optFlag("--author", i.author)]),
    ...
    "table cell copy": (i) => plain(["table", "cell", "copy", str(i.id), str(i.slidePath), str(i.elementId), "--range", str(i.range)]),
    "table cell cut": (i) => plain(["table", "cell", "cut", str(i.id), str(i.slidePath), str(i.elementId), "--range", str(i.range)]),
    "table cell paste": async (i) => {
        return {
            argv: ["table", "cell", "paste", str(i.id), str(i.slidePath), str(i.elementId), "--at", str(i.at), "--tsv-file", "-"],
            body: new TextEncoder().encode(str(i.tsv)),
            cleanup: NOOP_CLEANUP,
        };
    },
    "table create": (i) => plain([
    ...
    "table col insert": (i) => plain(["table", "col", "insert", str(i.id), str(i.slidePath), str(i.elementId), ...optFlag("--at", i.at)]),
    "table col delete": (i) => plain(["table", "col", "delete", str(i.id), str(i.slidePath), str(i.elementId), ...optFlag("--at", i.at)]),
    "table row insert": (i) => plain(["table", "row", "insert", str(i.id), str(i.slidePath), str(i.elementId), ...optFlag("--at", i.at)]),
    "table row delete": (i) => plain(["table", "row", "delete", str(i.id), str(i.slidePath), str(i.elementId), ...optFlag("--at", i.at)]),
    "table theme set": (i) => plain(["table", "theme", "set", str(i.id), str(i.slidePath), str(i.elementId), str(i.theme)]),
    "table header set": (i) => plain(["table", "header", "set", str(i.id), str(i.slidePath), str(i.elementId), i.header ? "true" : "false"]),
    "table bind": (i) => plain([
    ...
    "chart data set": (i) => {
        ...
        if (i.csv !== undefined)
            return plain([...base, "--csv", str(i.csv)]);
        if (i.csvAsset !== undefined)
            return plain([...base, "--csv-asset", str(i.csvAsset)]);
        const categories = Array.isArray(i.categories) ? i.categories.join(",") : "";
        const series = Array.isArray(i.series) ? i.series : [];
        return plain([
            ...base,
    ...
    "chart palette set": (i) => {
        const colors = Array.isArray(i.colors) ? i.colors : [];
        return plain([
    ...
    "chart axis set": (i) => {
        const right = Array.isArray(i.right) ? i.right : [];
        return plain([
            "chart", "axis", "set", str(i.id), str(i.slidePath), str(i.elementId), str(i.axes),
            ...right.flatMap((r) => ["--right", r]),
        ]);
    },
    "chart stack set": (i) => plain(["chart", "stack", "set", str(i.id), str(i.slidePath), str(i.elementId), i.stacked ? "on" : "off"]),
    "chart legend set": (i) => plain(["chart", "legend", "set", str(i.id), str(i.slidePath), str(i.elementId), str(i.legend)]),
    "chart option set": (i) => plain(["chart", "option", "set", str(i.id), str(i.slidePath), str(i.elementId), str(i.key), str(i.value)]),
    "textbox align": (i) => plain(["textbox", "align", str(i.id), str(i.slidePath), str(i.elementId), str(i.align), ...boolFlag("--force", i.force)]),
    "slide style set": (i) => plain(["slide", "style", "set", str(i.id), str(i.slidePath), ...optFlag("--background", i.background), ...optFlag("--accent", i.accent)]),
    "slide background set": (i) => plain([
        "slide", "background", "set", str(i.id), str(i.slidePath),
        ...optFlag("--asset", i.asset), ...boolFlag("--none", i.none), ...optFlag("--opacity", i.opacity),
    ]),
    "presentation canvas set": (i) => plain(["presentation", "canvas", "set", str(i.id), ...optFlag("--width", i.width), ...optFlag("--height", i.height)]),
};
...
export async function encodeCommandArgv(name, input) {
