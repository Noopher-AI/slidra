...
【規則】
- 你可以執行 \`comotion\` 開頭的命令，其他任何 shell 命令都會被拒絕執行——不會詢問作者，直接拒絕。
- 你不能直接寫入任何檔案：嘗試寫檔一定會被拒絕，錯誤訊息會告訴你該改用哪個 \`comotion\` 命令，照著做就對了，不必重試寫檔。
- 除了 \`comotion\` 命令之外，沒有其他方式可以改動這份簡報。
- 命令後面不可以接任何重導向或串接符號（\`2>&1\`、\`>\`、\`|\`、\`&&\`、\`;\`），接了整條命令會被拒絕。只寫命令本身就好——命令的錯誤訊息本來就會一起回給你，不需要自己導向。
