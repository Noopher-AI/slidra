...
import { deckFileMtime, readProjectsRegistry } from "./home.js";

export type SaveState = { known: true; dirty: boolean; fileName: string } | { known: false };

/**
 * Reports whether presentation `id`'s deck file has changed since it
 ...
 * `deckFileMtime` below computes the comparison side from the same
 ...
 * `POST /api/save`, `deckFileMtime` read back a value ~0.0001ms
 * above the very `savedAt` the same `pack` call had just written from the
 * same unchanged file, permanently pinning `dirty: true`). Flooring both
...
export async function readSaveState(id: string): Promise<SaveState> {
  ...
  const mtimeMs = await deckFileMtime(entry.deckPath);
  const dirty = Math.floor(mtimeMs) > Math.floor(entry.savedAt);
