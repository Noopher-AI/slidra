 ...
 * commands ever need. `co-motion open`/`co-motion new` already run the
 ...
 * `packages/core`'s `presentation.ts` / `crates/co-motion/src/presentation.rs`
 * `FORMAT_VERSION` constant, both currently `4`). A freshly `new`+`open`ed
 * presentation is written at exactly this version, but an older `.comot`
 ...
 * *higher* than this build knows about is a real problem (a newer CoMotion
 ...
 * ever reads what `co-motion` already validated moments earlier. The
...
export function validateMinimalProjectJson(value: unknown): ProjectJson {
  ...
  if (record.formatVersion > CURRENT_FORMAT_VERSION) {
    throw new CoMotionError(`此簡報由較新版本的 CoMotion 建立（格式版本 ${record.formatVersion}），請升級後再開啟`);
  }
