...
配方檔裡的顏色寫成 `fill="var(--primary, #4F8DFF)"` 這種 CSS 變數語法，不是裸的 `<primary>`——後者的 `<` 在 SVG 屬性值裡不合法 XML，這些檔案現在是獨立的 `.svg`，直接用瀏覽器打開也要能通過 XML 解析。逗號後面那個色碼是**預設值**（沒代換也能直接打開看預覽，深底配方預設 01 `editorial-tech`、46–54 淺底配方預設 02 `warm-editorial`），不是要保留的東西——建資產時把整段 `var(--role, #預設色)` 換成這份簡報的實際色碼，不是只換逗號前面那半。
