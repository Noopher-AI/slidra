...
4. **一頁怎麼做**（六個階段，照順序；前一階段沒做完不要跳下一階段）
   ...
   1. **構圖思考**：計畫已經定好這一頁的 `relationship`（內容之間是什麼關係），**計畫不會給你 `type`，版面是你的決定**。照這個順序想，不要跳步：
