...
4. **一頁怎麼做**（每一頁都照這個順序）：
   1. 取該頁型在指南第 4 節的 SVG 範例，把每個 `<role>` 換成 design-spec 的色碼、範例文字換成計畫裡的關鍵詞（標題＝主張），卡片或條目依計畫的條數增減（座標公式在範例下方）。計畫 `background` 是 `on` 時，**再把第 4b 節該頁型的 scrim rect 加進去**（放在被它墊著的文字之前）。**所有文字都用文字框宣告**（`<text data-comot-text-width=…>`，內容直接換行分段），不要自己放 `<tspan>`；每個元素保留範例的 `id` 與 `data-comot-name`。
