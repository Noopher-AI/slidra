...
description: 從 25 種配色組與 10 種形狀語言裡各挑一種寫進 plan/design-spec.md（配色、字級表、字型、間距、形狀語言），用一句話描述想要的感覺就模糊匹配。作者的訊息以 /comotion-style-kit 開頭、或 comotion-plan 要適配風格時用
---
...
- `/comotion-style-kit` 後面的自由文字是匹配依據：氣質、場合、產業、顏色傾向、參考對象。
...
「建議背景」的編號是 `comotion-background-kit` 的配方；`off` 代表這個風格建議不要背景圖——乾淨本身就是它說服力的一部分。
