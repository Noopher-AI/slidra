# 12 · forest-field

**第一秒的感覺**：深苔綠配麥稈色。像走進一片被整理過的林地——有生命、有秩序，不是野生也不是人工。

**適合**：農業、食品產地、ESG、戶外品牌、地方創生。
**不適合**：都會科技、金融。綠色在這些主題裡會被讀成「環保口號」而不是主體。

**為什麼是這個配色**：主色是深苔綠（不是草綠——草綠太亮太塑膠）；accent 用麥稈黃，兩者是同一片土地上會同時出現的顏色，所以並置不吵；背景用極淺的米綠，白底會讓綠色浮起來像貼紙。

```json
{
  "density": "presentation",
  "palette": {"background": "#F5F7F0", "secondary_bg": "#E4EAD9", "primary": "#3E5C3A", "accent": "#C9A227", "secondary_accent": "#7A8B5A", "text": "#232B20", "muted": "#6B7563"},
  "type_scale": {"cover": 68, "section": 52, "number": 136, "claim": 46, "title": 38, "subtitle": 27, "body": 23, "column": 21, "caption": 17},
  "layout": {"side_margin": 80, "bottom_margin": 76, "footer_margin": 16, "gutter": 32, "spacing": [12, 20, 32, 48, 72]},
  "typography": {"heading": "Noto Serif TC", "body": "Noto Sans TC", "heading_weight": 700, "body_weight": 400},
  "shape_language": "plain",
  "visual": "editorial-tech"
}
```

**字型與對比**：標題襯線，有一點土地的老派感。內文黑體保持清楚。

**間距節奏**：級距鬆（12/20/32/48/72）。

**建議背景**：01 `soft-blobs` 或 02 `dot-grid`（opacity 0.3，像織紋）。
