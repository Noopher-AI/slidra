...
**Mood**: A large arc sweeping into the frame from the lower-right corner, like an enlarged seal mark. Quiet but present.

**Suited for**: `anchor`, `breathing`.
**Not suited for**: `dense`. The arc will clash with card borders.

**Suggested opacity**: 0.7

**Technique**: One large circle extending beyond the canvas, showing only the upper-left arc segment; stroke rather than fill, so it reads as a "line" not a "block."

Full SVG: see `corner-arc.svg` in the same folder.
...
**Suggested styles**: 01, 06, 13, 20, 23.
