...
2. **List every slide**: `slidra ls <presentation-id> slides`. There is no recorded link from a slide back to the template it was made from (ADR-0009: a template is dead the moment it's copied) — and `slide add --template` re-mints a fresh element id for every element it copies, so **element ids never match between a template and a slide made from it**. Match elements by `data-slidra-name` instead (unaffected by the copy — only `id` is regenerated); an element with no name, match by role and position (the same background rect, the same corner logo, the same footer text) the same way `/slidra-style` matches "the title" when nothing is explicitly named.
