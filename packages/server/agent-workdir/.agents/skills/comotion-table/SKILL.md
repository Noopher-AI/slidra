...
3. **建立表格元素**：`comotion table create <presentation-id> slides/00N.svg --rows <R> --cols <C> --x <N> --y <N>`（四個都必填）。沒指定座標時預設 `--x 140 --y 200`，回報裡說明「放在頁面偏左上，需要可以再調」。
4. **寫入內容**：記下步驟 3 回傳的 element id，`comotion table set <presentation-id> slides/00N.svg <el> --markdown '<多行 Markdown，含真正換行與對齊列>'`——單引號內放真正的換行，這是貼多行 Markdown 的唯一寫法。
   - 命令回「Markdown 表格第二列必須是對齊列」：替使用者補上對齊列（例如 `|---|---|`）再重送，回報裡說明補了什麼。
   - 命令回「第 N 列的欄數與標頭不符」：轉述原始錯誤、問使用者哪一列漏了；補空白欄會掩蓋資料問題。
5. **主題與標題列**：`comotion table theme set <presentation-id> slides/00N.svg <el> dark|light|zebra`；`comotion table header set <presentation-id> slides/00N.svg <el> true|false`。使用者說「striped」之類的字對應到 `zebra` 並說明對照。
