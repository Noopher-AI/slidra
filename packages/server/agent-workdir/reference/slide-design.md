...
| 只有小節名，底下沒有要點 | 章節頁 |
| 3～5 個並列的要點 | 要點頁 |
| 兩個東西的比較：A vs B、之前／之後、優點／缺點 | 對照頁 |
| 一個數字、一個百分比、或只有一句主張 | 大數字頁 |
| 最後一個小節是結論、行動、下一步 | 結語頁 |
| 6 條以上的要點 | 拆成兩張要點頁，標題加「（一）（二）」 |
| 流程、時間軸、表格等不在表上的關係 | 退回要點頁，並在回報裡建議作者這頁適合什麼版面 |

## 7. 內容密度：頁面只放主張與關鍵詞，說明進備忘稿

頁面是給台下**看**的，不是給人**讀**的；把要點寫成完整句子放上去，頁面就變成講稿。

| 項目 | 上限（density = presentation） |
|---|---|
| 標題（頁標題、章節名） | ≤ 15 字，1 行 |
| 封面大標 | ≤ 2 行，每行 ≤ 15 字 |
| 每條要點／每張卡片的關鍵詞 | ≤ 18 字，**1 行**，不加句號 |
| 要點條數 | 3～5 條（對照頁每欄 2～4 條） |
| 一頁的文字總量（所有文字框加總，含卡片編號，不含頁尾與裸文字浮水印；`{{ }}` 不計） | ≤ 600 字 |
| 大數字頁的說明、結語頁的結論 | ≤ 24 字 |
| 備忘稿 | 2～5 句，**這裡才放完整句子** |

- 一頁只講一個想法；標題是這一頁的主張（「本季營收成長 23%」），不是話題標籤（「營收」），除非作者的大綱本來就是話題式標題。
- **擴寫的去處是備忘稿**：作者的每一條要點，頁面上只留 1 行關鍵詞；講完整的那一兩句寫進 `slide notes set`。先說主張、再說依據、最後接到下一頁；不要念出顏色、位置、元素名。
