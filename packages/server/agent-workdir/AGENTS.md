# CoMotion Agent 工作手冊

這份文件鋪設在你（agent）的工作目錄裡，是穩定不變的長期指引——不像每次對話開頭收到的編輯規約，這份文件不需要每輪對話重講一次，你可以隨時用原生的檔案讀取能力回來查。

每次啟動 `co-motion serve` 都會用套件內建的版本整個重鋪這個工作目錄；你在這裡做的任何改動都不會被保留，也不應該嘗試在這裡留筆記給自己或使用者。

## 命令參考

編輯規約只示範少數幾個命令的語法。`co-motion` 完整的命令清單——每個命令的名稱、參數與一句用途——在 [`reference/commands.md`](reference/commands.md)。動手前先查那份參考，不要用編輯規約裡的少數範例去猜其他命令的語法。

## SVG 約定重點

<!-- T6/F5（NOOP-236，GitHub #237）在此填入 -->

## 工作慣例

<!-- T6/F5（NOOP-236，GitHub #237）在此填入 -->

## Skills

<!-- T6／T7 在此填入索引 -->

- `/comotion-animate` — 為指定頁或整份加上依序揭露的動畫效果與頁面轉場。
- `/comotion-chart` — 用一組數列在指定頁做出圖表，可選類型、雙軸與堆疊。
- `/comotion-notes` — 依每頁內容補上口語化的簡報者備忘稿，可指定時長。
- `/comotion-style` — 統一整份簡報的字級、顏色與字型，或把某一頁的樣式套到全部。
- `/comotion-table` — 把貼上的 Markdown 表格在指定頁做成表格，可綁定既有資料資產。
