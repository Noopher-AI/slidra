#!/usr/bin/env node
// The Rust `comotion` binary's only remaining Node entry point
// (`crates/comotion/src/node_entry.rs`) — `docs/spec/cli.md`'s `## 不經
// registry 的入口：serve 與 export`. Plain JS, deliberately outside
// `tsc -b`'s project-reference graph (same reason the deleted
// `packages/cli/bin/comotion.js` shim was): it is the runtime-only edge
...
if (argv[0] === "serve") {
  ...
  console.error("這個入口只接受 serve 與 export，其餘命令請直接執行 comotion 二進位");
  process.exitCode = 1;
}
