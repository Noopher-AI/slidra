...
describe("comotion/argv.ts: encodeCommandArgv", () => {
...
describe("comotion/argv.ts: COMMAND_WHITELIST ⇔ ARGV_ENCODERS key set equality", () => {
...
describe("comotion/command.ts: runJsonCommand (envelope parsing, exit-code-blind)", () => {
  ...
  it("ok:true envelope, exit 0 → {ok:true, data, message}", async () => {
    ...
    const result = await runJsonCommand(["open", "/tmp/x.comot"]);
  ...
  it("ok:true envelope but non-zero exit code — still read as ok:true (e.g. an EPIPE after real work finished)", async () => {
    ...
    const result = await runJsonCommand(["pack", "p1", "/tmp/out.comot"]);
...
describe("comotion/reads.ts: read decoding", () => {
  let fakeBinDir: string;
  let fakeBinPath: string;
  let previousBin: string | undefined;
