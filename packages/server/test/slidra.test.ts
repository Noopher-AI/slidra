...
import { ARGV_ENCODERS, COMMAND_WHITELIST, encodeCommandArgv } from "../../web/src/command-argv.js";
...
describe("web command-argv.ts: encodeCommandArgv", () => {
  ...
  it("element paste carries `svg` in the request body and appends --svg-file - last, honouring dx:0/dy:0", async () => {
    ...
    expect(encoded.argv[encoded.argv.length - 1]).toBe("-");
    expect(new TextDecoder().decode(encoded.body)).toBe("<svg>pasted</svg>");
    await encoded.cleanup();
  });

  it("table cell paste carries `tsv` in the request body and appends --tsv-file - last", async () => {
    ...
    expect(encoded.argv[encoded.argv.length - 1]).toBe("-");
    expect(new TextDecoder().decode(encoded.body)).toBe("a\tb\nc\td");
    await encoded.cleanup();
  });
