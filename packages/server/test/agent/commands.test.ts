...
describe("Seam B: GET /api/agent/commands and the agent-commands SSE event", () => {
  ...
  async function serve(agent: AgentAdapterConfig): Promise<RunningServer> {
    await registry.dispatch("new", { path: path.join(comotDir, "deck.comot"), name: "測試簡報" });
    const opened = await registry.dispatch<{ id: string }>("open", { path: path.join(comotDir, "deck.comot") });
