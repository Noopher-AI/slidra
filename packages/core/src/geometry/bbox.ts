...
function textPrimitiveBounds(primitive: SlidePrimitive, options: PrimitiveBoundsOptions): Rect {
  ...
  const fontWeightRaw = primitive.attrs.get("font-weight");
  const fontWeight = fontWeightRaw === undefined ? undefined : numberAttr(primitive, "font-weight", null);
  const style: TextStyle = { fontFamily, fontSize, fontWeight };
  const face = options.fontBook.faceFor(style);
  ...
  if (options.textWidth !== undefined) {
    ...
    const lineHeight = ((face.ascender - face.descender + face.lineGap) / face.unitsPerEm) * fontSize;
    const lines = Math.max(primitive.tspanCount, 1);
    return { x: 0, y: 0, width: options.textWidth, height: lines * lineHeight };
  }

  const width = options.fontBook.measureText(primitive.text, style);
