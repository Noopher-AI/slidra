...
/** Reads the style a `<text>` node's own attributes declare, for re-wrapping a text box's content. */
function readTextStyle(textNode: ScannedNode, elementId: string): TextStyle {
  ...
  const fontWeightRaw = attributeValue(textNode, "font-weight");
  let fontWeight: number | undefined;
  if (fontWeightRaw !== null && fontWeightRaw.trim() !== "") {
    fontWeight = Number(fontWeightRaw);
    if (!Number.isFinite(fontWeight)) {
      throw new CoMotionError(`文字框的 font-weight 不是合法的數字，無法重新換行：${elementId}`);
    }
  }
...
function replaceContainerText(
  ...
  if (textWidthRaw !== null) {
    ...
    const style = readTextStyle(textNode, elementId);
    const wrapped = wrapText(newText, { width, style, book: options.fontBook });
...
export function resizeTextBox(
  ...
  const sourceText = tspans
    ...
    .join("");
  const style = readTextStyle(textNode, elementId);
  const wrapped = wrapText(sourceText, { width: roundedWidth, style, book: fontBook });
  const content = renderTextBoxContent(wrapped.lines);
