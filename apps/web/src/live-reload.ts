...
/**
 * NOOP-93 §4.4 — restated here rather than imported: `@slidra/server`'s
...
/** [E3.T5] NOOP-230: the two adapters `AgentManager` ever reports — restated here (not imported from `@slidra/server`) for the same reason `ExportFormat` above is: the browser bundle must never depend on a Node-only package. */
