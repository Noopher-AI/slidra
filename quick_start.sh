...
驗收清單（這一輪做完的部分）：
  ...
  播放模式 #28
    - 翻到第 3 頁（有效果清單那頁），按「播放」：畫面立刻不會閃過完整內容——
      「步驟一」「步驟二」「步驟三」三行文字一開始就是隱藏的，標題維持可見。
    - 按方向鍵右，三行文字依序出現，每按一次只出現一行。
    - 在最後一步再按一次方向鍵右，會換到下一頁（此時已是最後一頁，不動也不當機）。
    - 按方向鍵左：不會倒退任何一步。
...
還沒有 UI、只能用測試查驗的部分：
  ...
  - 影音效果 #30：播放模式目前只實作 enter 家族（fade/appear），media 家族的
    play 效果解析後不會被套用，屬於下一輪的範圍。
