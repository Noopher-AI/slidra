...
| 路徑 | 內容 |
|---|---|
| `prototype/` | 可執行原型（`CoMotion (New v3).dc.html` + `comotion-logic-v3.js` + `support.js` + 字型）。`reference/` 內為舊版重建與 v1 對照 |
...
## 執行原型
原型以 ES module 動態載入 `comotion-logic-v3.js`，需透過 HTTP 伺服器開啟（不能直接 file://）：

```bash
cd prototype
python3 -m http.server 8080
# 開 http://localhost:8080/CoMotion%20(New%20v3).dc.html
```

## 原型結構
- `CoMotion (New v3).dc.html`：宣告式模板（所有樣式 inline），`<x-dc>` 內為 UI；底部 `<script data-dc-script>` 為極薄的殼，動態載入邏輯。
- `comotion-logic-v3.js`：`INITIAL_STATE` + `Logic` class（狀態、歷史、元素、表格、圖表 SVG、動畫、群組、畫布縮放、插入面板、AI 留言）。`renderVals()` 把狀態攤平成模板可讀的值。
