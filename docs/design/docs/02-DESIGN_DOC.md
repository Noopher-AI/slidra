...
CoMotion is a presentation tool built for "human and AI agent co-editing." Each slide is itself an SVG file (animation expressed as `<style>` keyframes/SMIL inside the SVG); the agent modifies files through CLI commands, while the human works directly in the GUI and gives the agent context through "comments."
...
```
┌ Titlebar 48 ───────────────────────────────────────────────────────────┐
│ CoMotion BETA │ ↶ ↷ │ filename.comot  Saved  │        Open Save Export ▶Play│
