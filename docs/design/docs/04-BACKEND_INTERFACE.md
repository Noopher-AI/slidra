...
The prototype is frontend-only; what follows is a backend contract derived from the frontend data model and interactions, for implementation reference. The core premise: **a slide is an SVG file**. A deck is a folder (with `.slidra` as the manifest), the agent operates on files via the CLI, and the GUI and the agent stay in sync through the same files and event stream.

## 1. File layout
```
Q3 Product Roadmap.slidra/       # or a single zip
