...
- Rebuilt the old version as `CoMotion (Old)` for comparison first, then produced `CoMotion (New)`.
...
- Title bar: moved Undo/Redo to the left of the filename; filename now shows `.comot`; the logo changed from "CM" to removed entirely; "agent connected" indicator removed; AI avatar changed to 🤖.
