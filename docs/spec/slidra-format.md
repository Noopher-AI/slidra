...
這份文件是 `.slidra` 容器格式與 `~/.slidra/`（`SLIDRA_HOME`）工作區佈局的唯一規範性文件。TypeScript 引擎已刪除（[E4.T12]）——Rust 是現在唯一的實作，本文件即它的唯一依據；`formatVersion` 4 與 1→4 遷移規則已由 Rust 版落地。

`docs/adr/` 記錄的是決策史（為什麼當初這樣選），本文件記錄的是目前與未來的結構性事實。兩者衝突時以本文件為準。

## 容器佈局

`.slidra` 是一個 zip 檔，沒有壓縮層級的契約（現行實作用 `fflate` 的 `level: 6`，這是實作選擇，不是格式的一部分，Rust 版可以用任何壓縮層級或甚至不壓縮）。
...
解壓安全規則（`unpack_container`，`crates/slidra/src/container.rs`）：**先全量驗證，再落地**。zip 裡的每一個 entry 路徑，只要是絕對路徑，或解析後會落在目標目錄之外（也就是含有能跳出目標目錄的 `..` 片段），整個壓縮檔就被拒收——不做部分解壓、不做路徑清洗、不嘗試修正。任何一步失敗（entry 路徑不合法、`project.json` 缺失或格式錯誤、`formatVersion` 太新），已經寫出的目標目錄會被整個刪除，不留下半成品。

## `project.json`

`.slidra` 唯一的中繼資料檔，UTF-8 JSON，鍵值如下：
...
每個 `.svg` 檔本身就是成品，不是要再轉檔的中間格式（ADR-0001）。可編輯的圖元包在 `<g id="el-…" data-slidra-name="…">` 容器裡（ADR-0012）；裸圖元（沒有 `<g>` 包住的 `<text>` 等）視為不合規。
...
`data-slidra-*` 屬性總表（目前 repo 內實際出現的完整集合，`grep -rhoP 'data-slidra-[a-zA-Z-]+' crates/slidra/src`）：

| 屬性 | 用在哪裡 | 意義 |
|---|---|---|
| `data-slidra-name` | 每個元素容器 `<g>` | 使用者可見的顯示名稱（`element name set`），不影響 id |
| `data-slidra-type` | 圖表／表格容器 `<g>` | 標記容器走哪一種例外形狀，目前值有 `chart`、`table` |
| `data-slidra-lock` | 元素容器 `<g>` | 鎖定狀態（`element lock`／`unlock`）；`--force` 可繞過鎖定寫入 |
| `data-slidra-clipboard` | 內部使用 | 剪貼簿序列化交換格式的標記屬性 |
| `data-slidra-media` | 媒體元素（image／video／audio） | 標記元素攜帶的媒體種類 |
| `data-slidra-embed` | 媒體元素 | 是否內嵌（依 `element insert` 的 `--embed`） |
| `data-slidra-source` | 內部使用 | 追蹤某內容片段的來源標記 |
| `data-slidra-list` | 段落 `<tspan>`／文字節點 | 清單樣式（`text list set`）：`bullet`／`number`／`none` |
| `data-slidra-list-marker` | 清單項目 | 該項目實際渲染出的標記文字（例如 `•`、`1.`） |
| `data-slidra-text-align` / `data-slidra-text-width` / `data-slidra-text-height` | 文字框 | 文字框排版計算的快取／標記屬性 |
| `data-slidra-break` | `<tspan>` | 標記這個 tspan 後面是一個「硬換行」（原文有 `\n`），與軟換行（單純換行顯示但原文無 `\n`）區分 |
| `data-slidra-type="table"`、`data-slidra-cols`、`data-slidra-rows`、`data-slidra-header`、`data-slidra-theme` | 表格容器 `<g>` | 見下方「表格容器」 |
| `data-slidra-cell`、`data-slidra-span`、`data-slidra-repeat`、`data-slidra-generated`、`data-slidra-align` | 表格儲存格 `<g>` | 見下方「表格容器」 |

（此表以目前程式碼實際使用到的屬性為準，逐項精確驗證規則以對應的 `crates/slidra/src` 模組——`element/text.rs`、`element/edit.rs`、`table/model.rs`、`chart/model.rs`——為權威來源；本表是總覽，不是每個屬性驗證規則的完整重述。）

### 背景圖元素（#303 §13）

一張投影片最多有一個頂層容器帶 `data-slidra-role="background"`：固定 `id="el-background"`、`data-slidra-name="背景圖"`、`data-slidra-lock="true"`，內容是一個滿版的 `<image href="../assets/…" x="0" y="0" width="畫布寬" height="畫布高" [opacity]>`，位置在 `<metadata>` 之後、所有其他元素之前（最底層）。由 `slide background set` 寫入／替換／移除；`slide add --svg`／`slide set --svg` 的內容若已含這個容器，原樣保留並補鎖。`validate` 不對它套用任何幾何、樣式、禁忌規則，只用它的存在決定是否檢查 `structure.scrim`。

## `<metadata>` 內的 `slidra:*`

**只有下面這 6 種標籤活在 `<svg>` 的 `<metadata>` 子節點裡**：`slidra:effects`（包 `slidra:effect`）、`slidra:transition`、`slidra:notes`、`slidra:comments`（包 `slidra:comment`）。命名空間 URI 統一為 `https://slidra.app/ns/2026`（`EFFECTS_NS` 常數，`NOTES_NS`／`TRANSITION_NS`／`COMMENTS_NS` 皆取同一個字面值）。

> 圖表的 `<slidra:chart>`（包 `slidra:series`、`slidra:categories`）與表格的 `<slidra:source>`，雖然也叫 `slidra:*`、也用同一個命名空間，**並不在 `<metadata>` 裡**，而是分別直接掛在圖表／表格自己的 `<g>` 容器底下（ADR-0012 amendment 的例外形狀）。這點容易被誤讀成十種標籤都在 `<metadata>` 底下，實際上不是——見下面「圖表容器」與「表格容器」兩節。
...
一張投影片的 `<metadata>` 內最多一組 `<slidra:transition>`；出現兩組以上視為簡報已損毀。屬性：`enter`／`exit` ∈ `none | fade | slide | zoom`；`enter-duration`／`exit-duration` 為非負秒數。**缺席時的預設值**（沒有 `<slidra:transition>` 元素，或元素存在但個別屬性缺席）：`enter: { effect: "none", duration: 0.6 }`、`exit: { effect: "none", duration: 0.5 }`。

### `<slidra:notes>`

```xml
<metadata>
  <slidra:notes xmlns:slidra="https://slidra.app/ns/2026">記得先自我介紹</slidra:notes>
</metadata>
```
...
<metadata>
  <slidra:comments xmlns:slidra="https://slidra.app/ns/2026">
    <slidra:comment id="c-01" target="el-p1-title" author="作者" created="2026-09-01T00:00:00.000Z">這頁的標題要不要再大一點？</slidra:comment>
  </slidra:comments>
</metadata>
```

`slidra:comment` 屬性：`id`（不透明字串）、`target`（元素 id 或字面值 `page` 代表整頁）、`author`（選填）、`created`（ISO 8601 時間戳，寫入時產生，之後不再變動——`comment edit` 只改內容不改 `created`）。內容是留言文字本身。

## 圖表容器

ADR-0012 amendment 的第一個例外形狀：一個 `data-slidra-type="chart"` 容器裡恰好一個 `<slidra:chart>`（資料）加一個內嵌的渲染結果 `<svg>`（畫面），不是「一或多個圖元」。
...
<g id="el-chart1" data-slidra-type="chart" transform="translate(691.2 115.2)">
  <slidra:chart xmlns:slidra="https://slidra.app/ns/2026"
    ...
    <slidra:series name="營收" values="100,120,140" axis="left"/>
...
`<slidra:chart>` 屬性：`type` ∈ `bar|hbar|line|area|pie|donut`；`stacked` ∈ `true|false`（字面字串，只有 `bar|hbar|area` 三種類型的堆疊才有實質效果）；`axes` ∈ `single|dual`；`palette` ∈ `brand|cool|warm`；`legend` ∈ `none|bottom|right`；`grid`／`labels` ∈ `true|false`；`x-title`／`y-title` 為字串（缺席視為空字串）；`width`／`height` 為正數，必填。子節點：一或多個 `<slidra:series>`（`name` 字串、`values` 逗號分隔數字列、`axis` ∈ `left|right`、`color` 選填 `#RRGGBB`）；恰好一個 `<slidra:categories>`（`values` 逗號分隔字串列）。`<slidra:chart>` 不貢獻 `getBBox()`；元素的邊界框只由內嵌 `<svg>` 的 `width`/`height` 決定，渲染結果只由 `chart` 命令族重畫產生，其他命令（`element style set` 等）一律拒絕直接碰觸這個容器內部。

## 表格容器

ADR-0012 amendment 的第二個例外形狀：一個 `data-slidra-type="table"` 容器包一個可選的 `<slidra:source>`（資料綁定宣告）加多個 `<g data-slidra-cell="r,c">` 儲存格——**沒有專屬的 `<slidra:table>` 元素**。
...
容器屬性：`data-slidra-cols`／`data-slidra-rows` 為以空白分隔的正數列（欄寬／列高，使用者單位）；`data-slidra-header` = `"1"` 代表有標題列；`data-slidra-theme` ∈ `dark|light|zebra`（缺席時預設 `dark`）。`<slidra:source>` 選填，出現時 `src` 是 `assets/` 下 CSV 檔的虛擬路徑（`table bind` 綁定的來源，`table refresh` 讀它重新整理）；一個表格容器最多一個 `<slidra:source>`。

儲存格 `<g data-slidra-cell="row,col">`：`row`/`col` 0-based；`data-slidra-span="rowSpan,colSpan"` 選填（缺席等同 `1,1`）；`data-slidra-repeat="row"` 標記這是綁定表格的模板列（永遠搭配 `display="none"`，一個表格最多一列是模板列）；`data-slidra-generated="1"` 標記這格內容是 `table bind`/`table refresh` 從 CSV 自動產生；`data-slidra-align` ∈ `left|center|right`（缺席預設 `left`）。文字內容在 `<text><tspan>...</tspan></text>` 裡；儲存格底色是 `<rect>` 的 `fill`（`none` 或 `#RRGGBB`）與可選 `fill-opacity`（0 到 1）；文字顏色是 `<text>` 的 `fill`（必須是 `#RRGGBB`）；字重是 `<text>` 的 `font-weight`（100 到 900 的整百）。**表格網格必須被儲存格完整覆蓋，不得有洞，合併範圍不得重疊**（`validateTableModel` 的結構不變式）。
...
`slide add --svg`／`slide set --svg` 接受一種**輸入形式**：直接放在根 `<svg>` 底下、帶 `data-slidra-text-width` 的裸 `<text>`（內容以換行分段，可帶 `data-slidra-list`、`data-slidra-text-align`）。寫入時它一律被換成 `slides/` 一節描述的真正文字框結構（`<g data-slidra-text-width data-slidra-text-height transform>` 包 `<text>` 與 `<tspan>`）；`.slidra` 裡永遠不會存到一個帶 `data-slidra-text-width` 的裸 `<text>`。
...
**單獨開啟一張 SVG 時的降級**（ADR-0016 決定二）：`@font-face` 只在 Slidra 自己的 wrapper 文件內透過內部路由注入；被外部工具（瀏覽器直接開檔、Illustrator 等）開啟時，`font-family` 依 CSS 字型堆疊規則的效果就是降級成該環境的系統字型——畫面不會壞，但不保證與 Slidra 內看到的逐像素一致。這是刻意決定，不是缺陷：把字型 base64 內嵌進每張 SVG 的 `@font-face` 會讓一份 N 頁簡報的字型儲存成本變成 N 倍。

## `formatVersion`

`FORMAT_VERSION`（`crates/slidra/src/presentation.rs`）是 **4**，本規格定案的值。`formatVersion` 大於目前建置支援的最大值時整份拒絕開啟。
...
| 2→3 | 讀取 `project.json.transition`：若值恰為字面字串 `"fade"`，為每一張**還沒有** `<slidra:transition>` 的投影片寫入 `enter: { effect: "fade", duration: 0.4 }`、`exit: { effect: "none", duration: 0.5 }`（保留舊版「整份淡入」的視覺效果）；其他任何值（`undefined`、`"none"`、`""`、未知字串）一律不遷移、不寫入任何東西。動作完成後刪除 `transition` 欄位，`formatVersion` 改為 3 | `open`（`unpackContainer` 內，每次開啟都跑一次，遷移只在 `formatVersion < 3` 時發生） |
| 3→4 | `fonts` 缺席時補 `fonts: []`；`templates` 若仍含裸字串則升級為物件（1→2 沒處理到的防禦性補漏）；確認 `transition` 欄位已不存在（1→2→3 應已清除，此處是防禦） | `open` |

**repo 現況**（供 Rust 開發時測試 fixture 參考）：目前 27 份 `project.json` fixture 全部是 `formatVersion: 1`；其中 24 份沒有 `fonts` 欄位（3→4 遷移的 `fonts: []` 補值路徑會覆蓋這 24 份）；只有一份有 `templates` 且已是物件形狀；**沒有任何一份帶有 `transition` 欄位**——也就是說 2→3 遷移的 `"fade"` 分支在現有 repo fixture 裡完全沒有被覆蓋到，Rust 實作 3→4 遷移鏈時，需要自己造一份帶 `transition: "fade"` 且投影片沒有 `<slidra:transition>` 的測試檔，才能驗到這條分支。

## `~/.slidra/`（`SLIDRA_HOME`）

`resolveSlidraHome()` 的解析規則：`process.env.SLIDRA_HOME` 若設定就用它，否則預設 `~/.slidra`；**每次呼叫都重新讀取環境變數，不快取**（測試可以把它指到臨時目錄）。

```
<SLIDRA_HOME>/
├── projects.json              # 登記檔
├── work/<id>/                 # 解壓後的工作目錄（= .slidra 的內容）
├── history/<id>/
│   ├── stack.json
...
interface RegistryEntry {
  ...
  sourcePath?: string;   // open 或「就地重開」最後讀取的 .slidra 路徑；較舊的登記項可能沒有這欄
...
每個 presentation id 一份獨立檔案，位置在 `work/<id>/` 之外（打包 `.slidra` 時絕不會把剪貼簿內容一起打包進去）。這個「一個 id 一份檔案」的檔案佈局，本身就是「只能在同一份簡報內貼上」規則的完整實作機制——沒有額外的檢查程式碼，貼上時就是讀這個檔案，讀不到就是沒有東西可貼。

## 已知待改（本規格不修，逐條寫明現況與原因）

這些是目前程式碼裡已知但刻意先不修的既有狀態，Rust 版**維持現況**，不在本次規格內修正：

- **`xmlns:slidra` 是逐區塊宣告，不是在 `<svg>` 根宣告一次。** 每一個 `slidra:*` 根區塊（`<slidra:effects>`、`<slidra:transition>`、`<slidra:notes>`、`<slidra:comments>`、`<slidra:chart>`、`<slidra:source>`）各自帶自己的 `xmlns:slidra="https://slidra.app/ns/2026"` 宣告，而不是共用 `<svg>` 根元素上宣告一次的版本。
- **一份 SVG 可以有多個 `<metadata>` 區塊。** 目前的讀寫邏輯允許這種情況並且能正確處理，但這不是刻意設計出的規範，只是現況。
- **歷史遺留的命名空間錯誤會被就地修正。** 舊版 `element-clipboard.ts` 的貼上路徑曾經寫出錯誤的 `xmlns:slidra="https://slidra.app/ns/2026"`（正確值應為 `https://slidra.app/ns/2026`）；讀取端（`effects/edit.ts` 的 `withCorrectedNamespace`）偵測到這個錯誤值或缺席時會就地改寫成正確值。這個修正邏輯保留，因為歷史檔案可能還帶著這個錯誤命名空間。
