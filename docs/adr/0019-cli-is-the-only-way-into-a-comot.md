...
ADR-0004 的第二層是一份白名單：`session/request_permission` 只放行 `comotion` 開頭、
...
**一份簡報的內容只能透過 `comotion` 命令改動；其餘 shell 命令不受限制。**

具體地說，`session/request_permission` 拒絕一條命令，當且僅當它指名了下列路徑之一：

- `<COMOTION_HOME>` 底下的任何東西——所有簡報的工作目錄、復原歷史、`projects.json`、
  存檔狀態——**除了** `<COMOTION_HOME>/agent/<id>`，那是 agent 自己的工作目錄，必須保持
  可用；
- 任何 `.comot` 容器檔，不論它在磁碟上的哪裡。

其他一切放行：管線、重導向、串接、別的程式、雙引號、反斜線。`comotion` 命令自己也不再
受那組字元文法約束——`comotion text set … "第三季 財報"` 現在可以直接寫。

三件配套：

- **被擋下時回一句建議，不是一個錯誤。** 拒絕傳到 agent 那裡是「使用者拒絕了」，它會
  當成人類說不要而停手。CoMotion 改成告訴它該用哪個命令（`command-hints.ts`），並在
  回合被 adapter 中斷時把回合接回來、把建議當成下一個 prompt 送出去，最多兩次。
- **凍結／復原群組的判斷與文法脫鉤。** 開啟 history group 的條件改成「第一個字是
  `comotion`」，不再是「通過那組字元文法」，否則 `comotion undo X | head` 這種寫法會
  悄悄不進復原群組。
- **規約同步改寫。** 編輯規約與 `AGENTS.md` 原本教的是「只能跑 `comotion`、不能接重導向、
...
- **路徑檢查不是圍籬，是路標。** `protected-paths.ts` 用字串層的路徑比對，變數、glob、
  ...
  `.comot` 由 CLI 自己的視角重新打包、`validate` 對內容有最後裁量、復原快照只認命令。
