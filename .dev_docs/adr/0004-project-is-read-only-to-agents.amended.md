...
>
> **Amended (NOOP-472, [E6.T8] OS agent sandbox — ADR-0021).** Layer 3 ("no real-path leakage") gains an OS-enforced backstop **on macOS**: `packages/server/src/sandbox/policy.ts`'s `denyRead` is passed into the Seatbelt profile `srt-launcher.ts` builds, so `SLIDRA_HOME` and every registered deck's real path are refused at the OS level even if a real path were leaked. **On Linux**, the OS sandbox (`landlock-launcher.ts`) does not consume `denyRead` at all — a Landlock ruleset built only from `AccessFs::from_write` has no read-restriction concept — so layer 3 there is exactly what it always was: never printing an absolute path, nothing more. This also closes the leak the banner above (two paragraphs up) acknowledged: the agent's working directory is no longer `<SLIDRA_HOME>/agent` (a fixed, product-owned location) but a fresh per-`serve`-process scratch root under the OS temp directory (`SandboxRoot`, [`sandbox/sandbox-root.ts`](../../packages/server/src/sandbox/sandbox-root.ts)); knowing that path buys an agent nothing, since it grants no read access to anything under `SLIDRA_HOME` or any deck.
