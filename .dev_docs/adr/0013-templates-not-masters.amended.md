...
> **Amended (NOOP-472, re-confirming against [E6.T8]).** Master view mode and the "let the agent update the slides" action (`slidra-apply-master` skill) do not change the position taken here: a template is still dead the instant it's copied, with **no inheritance, no automatic sync, and no recorded slide→template link**. The skill sweeps a template's change onto existing slides on request, once, as an ordinary agent-driven edit — matching elements by `data-slidra-name` (unaffected by the id-regeneration a copy already does) rather than any persisted link, because there is none to persist. A lightweight "made from template X" source marker was considered and rejected even though it would have made matching more reliable: it is the first rung of the attribute-by-attribute override-tracking ladder this ADR already rejected in "Why not inheritance, master-style" — it would need its own staleness handling the moment a matched element is deleted or a slide diverges further, exactly the kind of bookkeeping this decision exists to avoid. See also ADR-0008 (a slide is self-contained): a marker recording a relationship to something outside the slide's own SVG would be a step back from that.
