...
| 0003 | The `.slidra` container | Partially superseded → 0016; the `transition` field was retired and `formatVersion` moved 2→3, then 3→4 under the old TypeScript engine. **Since the Rust rewrite, `FORMAT_VERSION` was reset to 1 with no migration chain** — see `docs/spec/slidra-format.md` (the current authority) |
