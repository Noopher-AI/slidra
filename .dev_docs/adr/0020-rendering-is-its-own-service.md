# ADR-0020 — Rendering is its own service
...
(ADR-0016) and the bundle to the editor's service, the only thing export still owns is the
...
**Consequences.**
- Rendering holds a read-only credential (ADR-0018) and can hold nothing else. Export is
