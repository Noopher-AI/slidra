...
<h1 align="center">Slidra - UX-AX Unified Editor with Slide Harness</h1>
...
Slidra is a monorepo with a Next.js and React web editor, a resident Node.js server, and a Rust CLI that owns all presentation-content reads and writes. `slidra serve` serves the editor alongside the open deck and connects its chat to Claude Code, Codex, or Pi through ACP. Pi uses OpenRouter and reads `OPENROUTER_API_KEY` from the `serve` process environment. The web editor never accesses a `.slidra` deck directly: it sends read requests and semantic edit commands to the application server, which executes the corresponding `slidra` CLI commands.
...
| Agent integration | Agent Client Protocol, TypeScript | Connects Claude Code, Codex, or OpenRouter-backed Pi to the resident server while keeping deck changes inside the CLI command surface. |
