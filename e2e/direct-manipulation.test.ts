...
it("⌘]／⌘[ 的鍵盤層序入口（element order up/down，焦點在 iframe 內）", async () => {
  const { server, registry, presentationId, cleanup } = await startServerFor();
  try {
    const page = await openApp(server);
    const before = await readSlide(registry, presentationId);
    ...
    let after = await readSlide(registry, presentationId);
    expect(after.indexOf("el-b")).toBeLessThan(after.indexOf('id="el-a"'));

    let undo = await registry.dispatch("undo", { id: presentationId });
    expect(undo.ok).toBe(true);
    expect(await readSlide(registry, presentationId)).toBe(before);
    ...
    undo = await registry.dispatch("undo", { id: presentationId });
    expect(undo.ok).toBe(true);
    expect(await readSlide(registry, presentationId)).toBe(before);
  } finally {
    await cleanup();
