 ...
 * agent 執行 `slidra undo` 後 GUI 同步」。四個 case（Plan §6.2）：
...
it("CLI ↔ GUI 同步：agent 執行 slidra undo（registry.dispatch(\"undo\")）後，不重新整理頁面 GUI 也自己變回原值", async () => {
  ...
  try {
    ...
    // 模擬 agent 下 `slidra undo`：直接呼叫 registry，不經過瀏覽器。
