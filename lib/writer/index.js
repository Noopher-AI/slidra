...
// builds a formatVersion 6 SQLite container, edits one in place, and converts
// a legacy deck (formatVersion 5 SQLite, 1-4 ZIP) to 6. Node-only — it needs
...
/**
 * Edits a SQLite deck in place, keeping every table and row it does not touch
 * (spec §1.1: writers preserve what they do not understand). The edit runs on
 * a copy that replaces the original only if `edit` succeeds. A legacy
 * formatVersion 5 deck is upgraded to 6 in the same transaction (spec §1.2).
 *
...
/** Moves a legacy formatVersion 5 deck, open in a transaction, to FORMAT_VERSION (spec §1.2). */
function upgrade(db, editor) {
  db.exec(`PRAGMA user_version = ${FORMAT_VERSION}`);
  editor.project = { ...editor.project, formatVersion: FORMAT_VERSION };
  editor.projectChanged = true;
}
...
/**
 * Converts a legacy deck to formatVersion 6 — the one conversion the spec
 * sanctions (§1.2): a formatVersion 5 SQLite deck by moving its versions to
 * 6, a ZIP deck (formatVersion 1–4) by building the SQLite container from
 * its entries. Writes `outFile` atomically; `outFile` may equal `zipFile`.
 */
export async function convertLegacyDeck(zipFile, outFile = zipFile) {
  ...
  if (!deck.legacy) throw new WriterError(`${zipFile} is already formatVersion ${FORMAT_VERSION}`);
  if (deck.container === "sqlite") {
    atomically(outFile, (temp) => {
      copyFileSync(zipFile, temp);
      const db = new DatabaseSync(temp);
      try {
        db.exec("PRAGMA journal_mode = DELETE");
        db.exec("BEGIN");
        const editor = new DeckEditor(db, deck.project);
        upgrade(db, editor);
        editor.flush();
        db.exec("COMMIT");
      } finally {
