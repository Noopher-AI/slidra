...
/** The core layout roles (format §4.9). */
const CORE_ROLES = ["background", "field", "node", "spine", "edge", "label", "garnish"];
/** A tool-specific role extension, `<prefix>:<name>` (format §4.9). */
const EXTENSION_ROLE = /^[a-z][a-z0-9-]*:[a-z][a-z0-9-]*$/;
...
function checkVocabulary(root, slidePath, add) {
  ...
  for (const el of elementsUnder(root)) {
    if (isElementContainer(el)) {
      // An invalid data-slidra-role is reported once, as role-unknown (checkElements).
      for (const problem of checkAttributes("element", attributesOf(el)).filter((p) => p.path !== "/data-slidra-role"))
...
/** Element-level rules: unique ids, the id shape, layout roles, one background, content inside containers. */
function checkElements(root, slidePath, add) {
  ...
  for (const el of elementsUnder(root)) {
    const role = el.getAttribute("data-slidra-role");
    if (role === null || CORE_ROLES.includes(role) || EXTENSION_ROLE.test(role)) continue;
    add("error", "role-unknown", `data-slidra-role "${role}" is neither a layout role (${CORE_ROLES.join(", ")}) nor a <prefix>:<name> extension.`, "format §4.9", {
      path: slidePath,
