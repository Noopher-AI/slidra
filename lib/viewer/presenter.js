...
//
// A deck read through a deck source that is not a file in memory
// (lib/viewer/source.js) arrives as the source's `presenter()` descriptor
// instead; the page serving this view turns it back into a source with
// `startPresenter({ openSource })`.
...
let position = null;
let started = false;
/** @type {((descriptor: unknown) => import("./source.js").DeckSource | Promise<import("./source.js").DeckSource>) | null} */
let openSource = null;

const timer = { startedAt: Date.now(), pausedAt: null, pausedTotal: 0 };

/**
 * Wires the presenter view onto its rendered page.
 * @param {{ openSource?: (descriptor: unknown) => import("./source.js").DeckSource | Promise<import("./source.js").DeckSource> }} [options]
 *   `openSource` opens this window's own deck source from the descriptor the
 *   audience window's source gave (its `presenter()`); without it, only decks
 *   read from a file can be shown here.
 */
export function startPresenter(options = {}) {
  if (started) return;
  started = true;
  openSource = typeof options.openSource === "function" ? options.openSource : null;
...
async function onMessage(message) {
  ...
  if (message.type === "deck" && !deck && (message.bytes instanceof ArrayBuffer || "descriptor" in message)) {
    try {
...
async function loadDeck(message) {
  const runtimeSource = await loadRuntime();
  const fileName = typeof message.fileName === "string" ? message.fileName : undefined;
  if (message.bytes instanceof ArrayBuffer) {
    deck = PlayableDeck.fromDeck(await openDeck(new Uint8Array(message.bytes), { fileName }));
  } else {
    if (!openSource) throw new Error("this page cannot open the deck's source");
    deck = await PlayableDeck.open(await openSource(message.descriptor), { fileName });
  }
...
async function describe(target) {
  ...
  let prepared;
  try {
    prepared = await current.slide(target.index);
  } catch {
    return;
  }
  if (position !== target || player !== current) return;
  ...
  let nextTitle = null;
  try {
    nextTitle = (await current.slide(shownIndex)).title;
  } catch {
    /* the slide shows its number only */
  }
  if (position !== target || player !== current) return;
