...
function policy(nonce) {
  ...
  return (
    `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src ${script}; ` + "style-src 'unsafe-inline'; img-src data: https:; media-src data: https:; font-src data:\">"
  );
}
...
export function staticDocument(markup, fonts, { lang = null } = {}) {
  return `<!doctype html><html${langAttribute(lang)}><head><meta charset="utf-8">${policy(null)}${fonts}${VIEWPORT_STYLE}</head><body style="background:#fff">${markup}</body></html>`;
}
 ...
 */
export function playDocument(markup, fonts, plan, startStep, runtimeSource, { lang = null } = {}) {
  ...
  return (
    `<!doctype html><html${langAttribute(lang)}><head><meta charset="utf-8">${policy(nonce)}${fonts}${VIEWPORT_STYLE}${hideStyle}${morphHold}</head>` +
