...
export class SqliteReader {
  ...
  constructor(bytes) {
    ...
    // SQLite's own minimum; the overflow arithmetic below assumes it.
    if (this.usableSize < 480) throw new SqliteFormatError(`invalid reserved space ${bytes[20]} for page size ${this.pageSize}`);
   ...
   */
  readTable(name) {
    return guarded(() => this.#readTable(name));
  }
  ...
  *#scanTable(rootPage) {
  ...
  #payload(offset, size) {
    if (size > this.bytes.length) throw new SqliteFormatError(`a record claims ${size} bytes, more than the whole file`);
  ...
  #decodeRecord(payload) {
    ...
    if (headerSize > payload.length) throw new SqliteFormatError("a record header runs past its payload");
    const types = [];
    let cursor = n;
    ...
    for (const type of types) {
      switch (type) {
        ...
        default: {
          ...
          if (body + length > payload.length) throw new SqliteFormatError("a record value runs past its payload");
...
export function readVarint(bytes, offset) {
  if (offset < 0 || offset >= bytes.length) throw new SqliteFormatError("a varint starts outside the data");
  let value = 0;
  for (let i = 0; i < 8; i++) {
    const byte = bytes[offset + i];
    if (byte === undefined) throw new SqliteFormatError("a varint runs past the end of the data");
    value = value * 128 + (byte & 0x7f);
    if ((byte & 0x80) === 0) return [value, i + 1];
  }
  if (bytes[offset + 8] === undefined) throw new SqliteFormatError("a varint runs past the end of the data");
  value = value * 256 + bytes[offset + 8];
  return [value, 9];
}

/**
 * Runs a read, turning the RangeErrors and TypeErrors a damaged file can
 * provoke (an offset outside the file, a value of the wrong type) into
 * SqliteFormatErrors, so callers see one kind of failure.
 */
function guarded(read) {
  try {
    return read();
  } catch (error) {
    if (error instanceof SqliteFormatError) throw error;
    throw new SqliteFormatError(`the database is malformed (${error instanceof Error ? error.message || error.name : String(error)})`);
  }
}
