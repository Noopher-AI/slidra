 ...
 * @typedef {{ id: string, kind: ItemKind, name: string | null, description: string | null, text: string, within: string[] }} ReadingItem
 *   `text` is what a reader presents: the name (and description) when there is one, otherwise the derived text.
 *   `within` lists the element containers it sits inside, outermost first (a step that reveals a group reveals these items).
...
export function slideAccessibility(root, deck = {}) {
  ...
  const visit = (parent) => {
    for (const el of elementChildren(parent)) {
      ...
      if (title) {
        items.push({ id, kind, name: title, description: desc, text: desc ? `${title}. ${desc}` : title, within: [...within] });
        continue;
      }
      if (kind === "group") {
        within.push(id);
      ...
      if (text) items.push({ id, kind, name: null, description: null, text, within: [...within] });
    }
  };
  visit(root);
...
/**
 * What revealing the element `id` says to assistive technology: the text of
 * the reading items that are it or sit inside it, in reading order.
 * @param {SlideAccessibility | null} model
 * @param {string} id
 */
export function textRevealedBy(model, id) {
  if (!model) return "";
  return model.items
    .filter((item) => item.id === id || item.within.includes(id))
    .map((item) => item.text)
    .join(". ");
}
