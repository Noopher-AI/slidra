...
/**
 * Draws a slide into a PNG `width` pixels wide.
 ...
 */
export async function renderSlidePng(markup, fontCss, canvasSize, width) {
...
export async function renderThumbnail(markup, fontCss, canvasSize, width = 480) {
  try {
    return URL.createObjectURL(await renderSlidePng(markup, fontCss, canvasSize, width));
  } catch (error) {
    // A browser that taints the canvas still shows the SVG image itself.
    const svg = error instanceof DOMException && error.name === "SecurityError" ? thumbnailSvg(markup, fontCss) : null;
    if (svg) return URL.createObjectURL(new Blob([svg], { type: "image/svg+xml" }));
    throw error;
  }
}
