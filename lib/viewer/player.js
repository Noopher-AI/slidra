...
import { fontFaceCss } from "./frame.js";
...
export class Player extends EventTarget {
  ...
  thumbnail(index) {
    ...
    if (!pending) {
      ...
      pending = this.thumbnailQueue.then(() => {
        if (this.deck !== deck) throw new Error("the deck was closed");
        return renderThumbnail(this.slide(index).markup, this.fontCss, deck.canvas);
      });
  ...
  async show(index, { startAt = "first", animate = true } = {}) {
    ...
    const startStep = startAt === "last" ? plan.steps.length - 1 : startAt === "current" ? Math.min(this.step, plan.steps.length - 1) : -1;
    this.step = startStep;
    ...
    this.frame.srcdoc = playDocument(prepared.markup, this.fonts, plan, startStep, this.runtimeSource, { lang: prepared.lang, allowRemote: this.allowRemote });
