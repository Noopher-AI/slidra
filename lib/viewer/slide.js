 ...
 *   links: Record<string, import("./links.js").Link>, warnings: string[], remote: string[]
 * }}
...
export function prepareSlide(deck, index) {
  ...
  const accessibility = slideAccessibility(root, { lang: typeof deck.project.lang === "string" ? deck.project.lang : null });
  ...
  const remote = resolveDeckReferences(deck, root, path);
...
/**
 * Inlines every deck-local URL in the slide (attributes, inline style,
 * `<style>` blocks) as a `data:` URL, and returns the network URLs it found
...
function resolveDeckReferences(deck, root, documentPath) {
  ...
  const resolveCss = (css) =>
    css.replace(CSS_URL, (match, quote, reference) => {
      if (isRemoteUrl(reference)) remote.add(reference.trim());
  ...
  for (const el of [root, ...elementsUnder(root)]) {
    for (const name of URL_ATTRIBUTES) {
      ...
      if (isRemoteUrl(value) && !(name === "data-slidra-media" && el.hasAttribute("data-slidra-embed"))) remote.add(value.trim());
      const url = inline(value);
      if (url) el.setAttribute(name, url);
    }
    const xlink = el.getAttributeNS(XLINK_NS, "href");
    if (xlink !== null && isRemoteUrl(xlink)) remote.add(xlink.trim());
    if (xlink !== null) {
