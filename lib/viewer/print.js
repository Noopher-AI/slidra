...
/**
 * A prepared slide's markup as the audience sees it after `step` (playback
 * §8); `null` or a slide without a plan gives its static (final) look.
 */
export function markupAtStep(prepared, step) {
  if (step === null || !prepared.plan) return prepared.markup;
  return hideIds(prepared.markup, hiddenAfterStep(prepared.plan, step));
}
 ...
 * @param {(index: number) => PrintSlide | Promise<PrintSlide>} slide prepared slide getter; the markup must load nothing (files as data: URLs)
 * @param {string} fontCss @font-face rules for the deck's fonts, sources as data: URLs
 * @param {PrintOptions} options
 * @typedef {{ markup: string, plan: any, notes: string, title: string | null }} PrintSlide
 */
export async function buildPrintout(deck, slide, fontCss, options) {
  // One slide at a time: a deck source serves each on request.
  const prepared = [];
  for (let index = 0; index < deck.slides.length; index++) prepared.push(await slide(index));
  ...
  entries.forEach((entry, n) => {
    ...
    const markup = markupAtStep(data, entry.step);
