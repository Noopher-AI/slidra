...
// path. Knows the current deck (SQLite, formatVersion 6) and both legacy
// forms, read-only (SQLite formatVersion 5, ZIP formatVersion 1-4; spec
// §1.2), and sniffs the header bytes, never the file extension.

import { slideIdOf } from "./links.js";
import { LruCache } from "./lru.js";
import { SqliteReader, isSqlite } from "./sqlite-reader.js";
import { ZIP_LIMITS, isZip, readZip } from "./zip-reader.js";

/** The formatVersion writers produce (spec §1.1). */
export const FORMAT_VERSION = 6;
/** The legacy SQLite formatVersion, read-only (spec §1.2). */
export const LEGACY_SQLITE_VERSION = 5;
...
/**
 * Default resource limits (format §17). Pass `limits` to openDeck to change them.
 * @typedef {{ maxDeckBytes: number, maxEntries: number, maxEntryBytes: number, maxTotalBytes: number }} DeckLimits
 */
export const DEFAULT_LIMITS = Object.freeze({ maxDeckBytes: 1024 * 1024 * 1024, ...ZIP_LIMITS });

/** How many characters of data: URLs a deck keeps ready (about 192 MB of UTF-16 strings). */
const DATA_URL_CACHE_CHARS = 96 * 1024 * 1024;
...
export class Deck {
  ...
  constructor(project, entries, info) {
    ...
    /** A legacy deck (formatVersion 1-5, spec §1.2): readable, but a writer converts it to FORMAT_VERSION. */
    this.legacy = project.formatVersion !== FORMAT_VERSION;
    this.textDecoder = new TextDecoder("utf-8");
    // Bounded: a deck's media as base64 can be larger than the deck itself; a
    // URL that falls out is rebuilt on its next use.
    this.dataUrls = new LruCache(DATA_URL_CACHE_CHARS, (url) => url.length);
    /** @type {DeckInfo | null} */
  ...
  /**
   * The position of the slide carrying `data-slidra-slide-id` = `id` (spec
   * §3), or -1. Built once per deck; a duplicated id resolves to its first slide.
   */
  slideIndexById(id) {
    if (!this.slideIds) {
      this.slideIds = new Map();
      this.slides.forEach((path, index) => {
        const slideId = slideIdOf(this.readText(path));
        if (slideId && !this.slideIds.has(slideId)) this.slideIds.set(slideId, index);
      });
    }
    return this.slideIds.get(id) ?? -1;
  }
 ...
 * @param {{ fileName?: string, limits?: Partial<DeckLimits> }} [options]
 * @returns {Promise<Deck>}
 */
export async function openDeck(bytes, options = {}) {
  const limits = { ...DEFAULT_LIMITS, ...options.limits };
  if (bytes.length > limits.maxDeckBytes) throw new DeckError(`This deck is ${formatSize(bytes.length)}, more than the ${formatSize(limits.maxDeckBytes)} this viewer opens.`);
  ...
  throw new DeckError("This is not a .slidra file: the header is neither SQLite (formatVersion 6, or legacy 5) nor legacy ZIP (formatVersion 1-4).");
}
...
function openSqliteDeck(bytes, options) {
  ...
  if (rows.length > limits.maxEntries) throw new DeckError(`This deck has ${rows.length} entries, more than the limit of ${limits.maxEntries}.`);

  const entries = new Map();
  for (const row of rows) {
    if (!isSafeEntryPath(row.path)) throw new DeckError(`unsafe entry path in deck: ${JSON.stringify(row.path)}`);
    if (row.data instanceof Uint8Array && row.data.length > limits.maxEntryBytes)
      throw new DeckError(`${row.path} is ${formatSize(row.data.length)}, more than the limit of ${formatSize(limits.maxEntryBytes)} for one entry.`);
...
async function openZipDeck(bytes, options) {
  let raw;
  try {
    raw = await readZip(bytes, options.limits);
  } catch (error) {
    throw new DeckError(`This file starts like a legacy ZIP deck but cannot be read (${error.message}).`);
  }
  const entries = new Map();
...
export function readProject(entries, container) {
  ...
  if (container === "sqlite" && version !== FORMAT_VERSION && version !== LEGACY_SQLITE_VERSION) {
    throw new DeckError(`A SQLite deck must be formatVersion ${FORMAT_VERSION} (or legacy ${LEGACY_SQLITE_VERSION}), but this one says ${version}.`);
  }
