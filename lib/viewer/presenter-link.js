...
/**
 * A laser-pointer message from a presenter view: a point as fractions of the
 * slide, or `off`. Anything else is ignored.
 * @returns {{ x: number, y: number } | "off" | null}
 */
export function readLaser(message) {
  if (!message || message.type !== "laser") return null;
  if (message.off === true) return "off";
  const inside = (n) => typeof n === "number" && Number.isFinite(n) && n >= 0 && n <= 1;
  return inside(message.x) && inside(message.y) ? { x: message.x, y: message.y } : null;
}
