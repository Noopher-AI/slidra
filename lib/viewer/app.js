...
async function renderLibraryThumb(entry, thumb, title, meta) {
  try {
    ...
    meta.textContent = `${libraryDeck.slides.length} slide${libraryDeck.slides.length === 1 ? "" : "s"} · ${formatBytes(entry.size)}`;
    if (libraryDeck.slides.length === 0) return;
    const prepared = prepareSlide(libraryDeck, 0);
    const iframe = document.createElement("iframe");
    iframe.setAttribute("sandbox", "");
    iframe.setAttribute("aria-hidden", "true");
    iframe.tabIndex = -1;
    iframe.srcdoc = staticDocument(prepared.markup, fontFaceStyle(libraryDeck));
...
async function startDeck(opened, startIndex) {
  ...
  els.deckMeta.textContent = [deck.fileName, `${deck.canvas.width}×${deck.canvas.height}`, deck.container === "zip" ? `legacy v${deck.project.formatVersion}` : `v${deck.project.formatVersion}`]
    .filter(Boolean)
    .join(" · ");
  ...
  thumbObserver = new IntersectionObserver(
    (entries) => {
      for (const entry of entries) {
        ...
        iframe.tabIndex = -1;
        try {
          iframe.srcdoc = staticDocument(player.slide(index).markup, fonts);
        } catch (error) {
