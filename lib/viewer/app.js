...
async function printDeck(options) {
  ...
  let printout;
  try {
    printout = await buildPrintout(deck, (index) => player.slide(index), player.fontCss, options);
  } catch (error) {
    hideLoading();
...
async function exportCurrentSlide() {
  ...
  try {
    ...
    const png = await renderSlidePng(markupAtStep(prepared, player.step), player.fontCss, deck.canvas, exportWidth());
...
async function exportAllSlides() {
  ...
  try {
    for (let index = 0; index < deck.slides.length; index++) {
      ...
      const png = await renderSlidePng(player.slide(index).markup, player.fontCss, deck.canvas, exportWidth());
