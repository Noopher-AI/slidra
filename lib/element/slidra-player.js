...
// slideCount, deck, source (settable: a deck source, lib/viewer/source.js,
// played instead of `src`). Events: "slidechange" and "stepchange" (detail
...
import { PlayableDeck } from "../viewer/source.js";
...
export class SlidraPlayerElement extends HTMLElement {
  ...
  constructor() {
    ...
    this.loaded = null;
    /** @type {import("../viewer/source.js").DeckSource | null} */
  ...
  connectedCallback() {
    ...
    if (!this.loaded && (this.deckSource || this.getAttribute("src"))) this.load();
  }
  ...
  attributeChangedCallback(name, previous, value) {
    ...
    if (name === "src") {
      // A new `src` replaces a source set from script.
      this.deckSource = null;
      this.load();
    } else if (name === "allow-remote" && this.player) this.player.setAllowRemote(value !== null);
  }

  /** The loaded deck: the Deck (lib/viewer/deck.js) for a `src` file, the opened source (lib/viewer/source.js) for a `source`; or null. */
  get deck() {
    return this.loaded ? (this.loaded.bytesDeck ?? this.loaded) : null;
  }

  /**
   * The deck source being played, when one was set; setting one plays it
   * instead of `src`: the element then reads project.json, and only the
   * slides and files it shows, from the source. Setting null goes back to `src`.
   * @returns {import("../viewer/source.js").DeckSource | null}
   */
  get source() {
  ...
  async load() {
    ...
    try {
      const [deck, runtime] = await Promise.all([source ? PlayableDeck.open(source) : this.fetchDeck(src), this.runtimeSource()]);
  ...
  /** Downloads and opens the `.slidra` file at `src`. */
  async fetchDeck(src) {
    const deckUrl = new URL(src, document.baseURI).href;
    const response = await fetch(deckUrl);
    if (!response.ok) throw new Error(`could not download the deck (HTTP ${response.status})`);
    return PlayableDeck.fromDeck(await openDeck(new Uint8Array(await response.arrayBuffer()), { fileName: deckUrl.split("/").pop() }));
  }
