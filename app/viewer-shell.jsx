...
export default function ViewerShell() {
  ...
  return (
    <>
      ...
      <main id="home" className="home" hidden>
        <header className="masthead">
          <a className="wordmark" href="/" aria-label="Slidra home">
            <svg viewBox="0 0 32 32" aria-hidden="true"><rect x="2" y="6" width="28" height="20" rx="4" /><path d="M13 11.5v9l7.5-4.5z" /></svg>
            <span>Slidra</span>
          </a>
          <nav className="masthead-links">
            <a href="/spec/slidra-format.md" target="_blank" rel="noopener">Format spec</a>
            <a href="/spec/playback.md" target="_blank" rel="noopener">Playback spec</a>
          </nav>
        </header>
        ...
        <section className="hero">
          <div className="hero-copy">
            ...
            <h1>Open a deck.<br /><em>Press play.</em></h1>
            <p className="lede">A <code>.slidra</code> file is one SQLite database holding SVG slides, their animations, media and fonts. Drop one here and it plays in your browser. Nothing is uploaded.</p>
            <div className="hero-actions">
              <label className="button primary" htmlFor="file-input">
                <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M10 3v10m0-10L6 7m4-4 4 4M4 13v2.5A1.5 1.5 0 0 0 5.5 17h9a1.5 1.5 0 0 0 1.5-1.5V13" /></svg>
          ...
          <div id="dropzone" className="dropzone" tabIndex="0" role="button" aria-label="Drop a .slidra file here, or press Enter to choose one">
            <div className="dropzone-screen">
              <div className="dropzone-glyph" aria-hidden="true">
                <svg viewBox="0 0 64 64"><circle cx="32" cy="32" r="30" /><path d="M26 21v22l18-11z" /></svg>
              </div>
              <p className="dropzone-title">Drop a <span>.slidra</span> file</p>
        ...
        <footer className="colophon">
          <span>Slidra — the open <code>.slidra</code> format and viewer.</span>
          <span className="keys">Play: <kbd>→</kbd> <kbd>Space</kbd> next · <kbd>←</kbd> back · <kbd>G</kbd> overview · <kbd>N</kbd> notes · <kbd>F</kbd> fullscreen</span>
        </footer>
      </main>
      ...
      <section id="viewer" className="viewer" hidden aria-label="Presentation">
        ...
        <div id="controls" className="controls chrome" role="toolbar" aria-label="Playback">
          ...
          <button id="overview-button" className="icon-button" type="button" aria-label="Overview" title="Overview (G)">
            <svg viewBox="0 0 20 20" aria-hidden="true"><rect x="3" y="3" width="6" height="6" rx="1.2" /><rect x="11" y="3" width="6" height="6" rx="1.2" /><rect x="3" y="11" width="6" height="6" rx="1.2" /><rect x="11" y="11" width="6" height="6" rx="1.2" /></svg>
          </button>
          <button id="notes-button" className="icon-button" type="button" aria-label="Speaker notes" title="Speaker notes (N)" aria-pressed="false">
            <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M4 4.5h12M4 8.5h12M4 12.5h8" /><path d="M13 13.5l3 3" /></svg>
          </button>
        ...
        <div id="progress" className="progress" aria-hidden="true"><span id="progress-bar"></span></div>

        <aside id="notes-panel" className="notes-panel" hidden aria-label="Speaker notes">
          <div className="notes-head"><span>Speaker notes</span><span id="notes-slide"></span></div>
      ...
      <div id="loading" className="loading" hidden><span className="spinner" aria-hidden="true"></span><span id="loading-text">Opening deck…</span></div>
      <div id="toasts" className="toasts" role="status" aria-live="polite"></div>
      <div id="drag-veil" className="drag-veil" hidden><p>Drop to play</p></div>
    </>
  );
}
