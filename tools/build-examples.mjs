...
function minimal(showcaseEntries) {
  ...
  slides.push(slide({
    ...
    body: [
      ...
      mEl("maxis", "Axis labels", `${mText(900, 520, 15, M.faint, "x →", ` font-family="${M.mono}"`)}${mText(868, 244, 15, M.faint, "y", ` font-family="${M.mono}"`)}${mText(1044, 300, 15, M.red, "(132, 88)", ` font-family="${M.mono}"`)}`),
  ...
  slides.push(slide({
    ...
    body: [
      ...
      ...agenda.map(([title, desc], i) => {
        const y = 262 + i * 92;
        return mEl(`magenda${i}`, `Agenda ${i + 1}`, [
  ...
  slides.push(slide({
    ...
    effects: [
      effect(id("mbig"), "enter", "zoom", "on-click", { duration: "0.7" }),
      effect(id("mcaption"), "enter", "fly-up", "after-previous", { duration: "0.5" }),
    ],
    body: [
  ...
  slides.push(slide({
    ...
    effects: [
      effect(id("mleft"), "enter", "fly-up", "on-click", { duration: "0.5" }),
      effect(id("mright"), "enter", "fly-up", "on-click", { duration: "0.5" }),
    ],
    body: [
      mKicker("mkick", 96, 92, "COMPARE"),
      mHeading("Where does the title end up?"),
      column("mleft", 96, false, "HTML + CSS", "Position is emergent", ['<h1 class="title">', "  Quarterly Review", "</h1>"], ["…it depends on fonts, the viewport,", "the flow and every element before it."]),
  ...
  slides.push(slide({
    ...
    body: [
      ...
      mEl("mbox", "Box", `<rect x="0" y="0" width="240" height="120" rx="8" fill="${M.tint}" stroke="${M.red}" stroke-width="2.5"/>\n    ${mText(120, 68, 20, M.red, "el-hero", ` text-anchor="middle" font-family="${M.mono}" font-weight="700"`)}`, `translate(${bx} ${by})`),
      mEl("mdims", "Dimensions", [
  ...
  slides.push(slide({
    ...
    body: [
      ...
      ...checks.map(([title, formula, desc], i) => {
        const x = 96 + i * 278;
        return mEl(`mcard${i}`, `Check ${title}`, [
  ...
  slides.push(slide({
    ...
    effects: [
      effect(id("mquote"), "enter", "fade", "on-click", { duration: "0.8" }),
      effect(id("mattrib"), "enter", "fly-up", "after-previous", { duration: "0.5" }),
    ],
    body: [
      mEl("mmark", "Quote mark", mText(84, 300, 240, M.red, "“", ' font-weight="700"')),
      mEl("mquote", "Quote", `${mText(96, 360, 60, "#ffffff", "The slide is the artifact —", ' font-weight="700"')}\n    ${mText(96, 438, 60, "#ffffff", "not a description of one.", ' font-weight="700"')}`),
      mEl("mattrib", "Attribution", [
        `<rect x="96" y="500" width="48" height="4" rx="2" fill="${M.red}"/>`,
        mText(160, 508, 22, "#b4a9a6", ".slidra design principle"),
      ].join("\n    ")),
    ].join("\n"),
  }));
  ...
  slides.push(slide({
    ...
    body: [
      ...
      ...groups.map(([name, , color], i) => mEl(`mlegend${i}`, `Legend ${name}`, [
  ...
  slides.push(slide({
    ...
    effects: [
      effect(id("mthanks"), "enter", "fly-up", "on-click", { duration: "0.7" }),
      effect(id("mthanksb"), "enter", "fade", "after-previous", { duration: "0.5" }),
    ],
    body: [
  ...
  const slidePaths = slides.map((_, i) => `slides/${String(i + 1).padStart(3, "0")}.svg`);
  const entries = [["slides", null], ["assets", null], ["fonts", null]];
  const extra = {};
  if (fontPath) {
    entries.push(["fonts/NotoSansTC-Presentation.ttf", readFileSync(fontPath)]);
    const licence = path.join(path.dirname(fontPath), "LICENSE-NotoSansTC.txt");
