...
這個目錄底下三份 deck，分別驗收 `.agents/skills/{outline,reshape,check}/SKILL.md` 三個主場景 skill。每個 deck 目錄是一份未打包的簡報（`project.json` ＋ `slides/00N.svg`，`outline-deck` 多一個 `INPUT.md`），各自搭配一份 `EXPECTED.md` 描述跑完該 skill 之後應該長成什麼樣子。
...
| 目錄 | 驗證的 skill | 說明 |
|---|---|---|
| `outline-deck/` | `/comotion-outline` | `INPUT.md` 是一份 5 段大綱，`EXPECTED.md` 描述應該長成的 5 頁投影片 |
| `reshape-deck/` | `/comotion-reshape` | 3 頁、預埋 5 則留言（3 則可處理、2 則不可處理），`EXPECTED.md` 逐則列出處理結果 |
| `check-deck/` | `/comotion-check` | 5 頁、預埋 6 類結構問題（每類一個），`EXPECTED.md` 列出應被釘留言的 (頁, target) 配對 |
