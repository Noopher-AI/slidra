...
- **除了 `<comot:comments>` 之外，五頁的其他內容必須完全不變。** 驗法：對每一頁 `cat <presentation-id> slides/00N.svg` 取回內容，把 `<metadata><comot:comments xmlns:comot="…">…</comot:comments></metadata>` 這一段整段拿掉（執行前這些頁面本來就沒有 comments，所以拿掉的應該是這次新加的那一段），剩下的內容要跟這個 fixture 目錄裡對應的原始檔案逐位元組相同。
